...
# ShrinkMe API key (hardcoded)
SHRINKME_API_KEY = "65aa5be4d757fb7242fff9dde00f6cd5d4acc977"
...
BOT_MENU_COMMANDS = [
    ...
    BotCommand("hotstar", "Open Hotstar interactive recording"),
    ...
    BotCommand("serverfiles", "Manage server-saved files"),
...
# Keep cancelled/partial server copies for 2 hours after Telegram upload.
PARTIAL_SERVER_COPY_TTL_SECONDS = 2 * 60 * 60
...
# /rec and protected commands are available in these explicitly allowed groups.
# Anonymous Admin: Telegram represents an anonymous admin as the group itself
# (sender_chat == chat). Therefore the configured group ID is used as the
# anonymous-admin access identity; no user token is required in this group.
try:
    _configured_group_id = int(getattr(config, "GROUP_CHAT_ID", GROUP_CHAT_ID))
except (TypeError, ValueError):
    _configured_group_id = int(GROUP_CHAT_ID)
ALLOWED_REC_GROUP_IDS = {_configured_group_id}
ANONYMOUS_ADMIN_GROUP_IDS = set(ALLOWED_REC_GROUP_IDS)
...
async def start_help_callback(client, query):
    ...
    text = (
        ...
        "• ⚡ `/drec LINK 00:00:30` - Direct stream-copy recording (video/audio copy)\n"
        ...
        "• 💾 `/ServerFiles` - Manage server-saved recordings/videos (2-hour retention)\n\n"
...
async def _probe_streams(url: str, provider: str | None = None):
    ...
    # Keep FFprobe access aligned with the direct PlaylistGen/FFmpeg path.
    # Some PlaylistGen endpoints reject the default FFprobe User-Agent with
    # HTTP 403, so use the same JioTV.Plus UA and protocol whitelist used by
    # the direct-remux recorder. Explicit configured/URL headers still win.
    cmd += [
        '-protocol_whitelist', 'file,http,https,tcp,tls,crypto',
        '-rw_timeout', '30000000',
        '-user_agent', 'JioTV.Plus/2.3.1 (Linux;Android 14)',
    ]
    # PlaylistGen action=stream returns a live MPEG-TS stream even though
    # the endpoint path ends in playlist.m3u. Do not force HLS demuxing here;
    # direct FFprobe succeeds when the response is detected automatically.
    is_playlistgen_stream = (
        'playlistgen-' in stream_url.lower()
        and 'action=stream' in stream_url.lower()
    )
    if not is_m3u8 and not is_mpd and not is_playlistgen_stream:
        cmd += ['-f', 'hls']
    provider_headers = _provider_headers(stream_url, provider)
    merged_headers = dict(provider_headers)
    # Explicit per-URL pipe headers take precedence over provider defaults.
    merged_headers.update(stream_headers)
    # A configured User-Agent must override the default JioTV.Plus UA.
    # Remove it from the generic header block and pass it through FFprobe's
    # dedicated -user_agent option so the argument list remains valid.
    configured_ua = None
    for k in list(merged_headers):
        if str(k).casefold() == 'user-agent':
            configured_ua = str(merged_headers.pop(k))
            break
    if configured_ua:
        cmd[cmd.index('-user_agent') + 1] = configured_ua
    if merged_headers:
        cmd += ['-headers', _format_stream_headers(merged_headers)]
    ...
    try:
        ...
        if proc.returncode != 0:
            probe_mode = (
                'playlistgen-auto' if is_playlistgen_stream
                else 'm3u8-auto' if is_m3u8
                else 'mpd-auto' if is_mpd
                else 'forced-hls'
            )
            LOG.warning('ffprobe failed (%s): %s',
                        probe_mode, err.decode(errors='ignore')[-1000:])
...
def _quality_keyboard(session):
    if session.get('sony_new_command') or session.get('hotstar_new_command'):
...
async def _verification_required(message) -> bool:
    """Allow normal verified users and Anonymous Admin in the allowed group."""
    user = getattr(message, "from_user", None)
    sender_chat = getattr(message, "sender_chat", None)
    chat = getattr(message, "chat", None)

    # Anonymous Admin messages are sent on behalf of the group.
    # Restrict this bypass to the configured authorized group(s).
    if (
        chat is not None
        and sender_chat is not None
        and getattr(sender_chat, "id", None) == getattr(chat, "id", None)
        and int(getattr(chat, "id", 0) or 0) in ANONYMOUS_ADMIN_GROUP_IDS
    ):
        return True
...
async def channel_command(client, message: Message):
    ...
    await message.reply_text(
        ...
        reply_markup=_playlist_keyboard()
    )
...
@app.on_callback_query(filters.regex(r"^playlist:(Tata Play|Airtel|FW)$"))
async def playlist_select_callback(client, query):
    playlist_name = query.data.split(":", 1)[1]
    await _safe_callback_answer(query, "🔄 Loading playlist...")

    items = await _playlist_items(playlist_name)

    if not items:
        return await query.message.edit_text(
            f"📺 **{playlist_name}**\n\n❌ Playlist load nahi ho saki.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ Back", callback_data="playlist_back")]
            ])
        )
    ...
    for index, (title, url, group) in enumerate(items):
        rows.append([
            InlineKeyboardButton(
                f"📺 {title}",
                callback_data=f"plch:{playlist_name}:{index}"
            )
        ])
    ...
    await query.message.edit_text(
        f"📺 **{playlist_name} Channels**\n\nSelect a channel:",
        reply_markup=InlineKeyboardMarkup(rows)
    )


@app.on_callback_query(filters.regex(r"^plch:(Tata Play|Airtel|FW):\d+$"))
async def playlist_channel_callback(client, query):
    ...
    try:
        ...
        return await _safe_callback_answer(
            query, "❌ Invalid channel.", show_alert=True
        )
    ...
    if index < 0 or index >= len(items):
        return await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(
                    "⬅️ Back",
                    callback_data=f"playlist:{playlist_name}"
                )]
            ])
        )
    ...
    flag = command_flags.get(playlist_name, "-FW")

    await query.message.edit_text(
        f"📺 **{title}**\n\n"
        f"Use this command:\n"
        f"`/rec {flag} {title} 00:00:30`",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "⬅️ Back",
                callback_data=f"playlist:{playlist_name}"
...
async def _capture_live_preview(url: str, output_path: str, provider: str | None = None) -> bool:
    ...
    try:
        ...
        preview_args = _apply_provider_headers(preview_args, url, provider)
        proc = await asyncio.create_subprocess_exec(
            *preview_args,
...
async def _run_scheduled_recording(client, item):
    try:
        ...
        heights, lang_indexes, selected_video_index, _probe_error = await _probe_streams(url, provider)
...
async def _probe_hotstar_quality_variants(url):
    return await _probe_sony_quality_variants(url)

def _hotstar_quality_keyboard(session):
    variants = session.get('hotstar_quality_variants') or []
    token = session.get('callback_token', '')
    rows = []

    if variants:
        best = max(variants, key=lambda x: (x[0], x[1]))
        rows.append([
            InlineKeyboardButton(
                f"⚡ Automatic • {best[0]}p",
                callback_data=f"hotq:{token}:auto"
            )
        ])

    for h, w, idx in sorted(variants, key=lambda x: (x[0], x[1])):
        rows.append([
            InlineKeyboardButton(
                f"{h}p • {w}×{h}",
                callback_data=f"hotq:{token}:{h}:{idx}"
            )
        ])
    ...
    rows.append([
        InlineKeyboardButton(
            "⬅️ Back: Audio Tracks",
            callback_data=f"hotback:{token}"
        )
    ])
    rows.append([
        InlineKeyboardButton(
            "❌ Cancel",
            callback_data=f"recs:{token}:cancel"
        )
    ])
    return InlineKeyboardMarkup(rows)

@app.on_message(filters.command("Hotstar"))
async def hotstar_command(client, message):
    try:
        parts = message.text.split(maxsplit=2)
        if len(parts) < 3:
            await message.reply_text(
                "❌ Format:\n/Hotstar 00:00:30 FileName"
            )
            return
        ...
        items = await _playlist_items("Jio Hotstar")
        if not items:
            await message.reply_text("❌ Hotstar channels unavailable.")
            return
        ...
        session = {
            ...
            'watermark': 'off',
            'step': 'channel',
            'sony_new_command': False,
            'hotstar_new_command': True,
            'hotstar_items': items,
            'hotstar_quality_variants': [],
            'quality_video_index': None,
        }

        rec_sessions[token] = session
        ...
        await message.reply_text(
            "🎬 **Hotstar Live Channels**\n\n📺 Select Channel:",
            reply_markup=InlineKeyboardMarkup(rows)
        )
    except Exception as e:
        LOG.exception("Hotstar command failed: %s", e)
        await message.reply_text("❌ Hotstar error.")

@app.on_callback_query(filters.regex(r"^hotch:"))
async def hotstar_channel_callback(client, query):
    try:
        _, token, idx = query.data.split(":", 2)
        session = rec_sessions.get(token)
        ...
        if (
            query.from_user.id != session.get('actor_id')
            and not (
                int(getattr(getattr(query.message, "chat", None), "id", 0) or 0) in ANONYMOUS_ADMIN_GROUP_IDS
                and int(getattr(getattr(query.message, "chat", None), "id", 0) or 0) == int(session.get('chat_id', 0) or 0)
            )
        ):
            await query.answer("Not your session.", show_alert=True)
            return

        items = session.get('hotstar_items') or []
        item = items[int(idx)]
        title, url, group = item

        await query.answer("Loading live preview…")

        session['raw_source_name'] = f"Hotstar {title}"
        session['source_url'] = url

        streams = await _probe_streams(url)
        if not streams:
            await query.message.edit_text("❌ Unable to capture live preview from this stream.")
            return
        ...
        preview_dir = tempfile.mkdtemp(prefix="hotstar_preview_")
        preview_path = os.path.join(preview_dir, "preview.jpg")

        preview_ok = await _capture_live_preview(url, preview_path)
        ...
        LOG.exception("Hotstar channel callback failed: %s", e)
        await query.answer("Hotstar error.", show_alert=True)

@app.on_callback_query(filters.regex(r"^hotq:"))
async def hotstar_quality_callback(client, query):
    try:
        ...
        variants = session.get('hotstar_quality_variants') or []
        if not variants:
            await query.answer("No real qualities detected.", show_alert=True)
            return

        if choice == "auto":
            h, w, idx = max(variants, key=lambda x: (x[0], x[1]))
        else:
            h = int(choice.split(":")[0]) if ":" in choice else int(choice)
            idx = int(query.data.rsplit(":", 1)[-1])
            matches = [v for v in variants if v[0] == h and v[2] == idx]
            if not matches:
                await query.answer("Quality unavailable.", show_alert=True)
                return
            h, w, idx = matches[0]

        session['quality'] = str(h)
        session['quality_video_index'] = idx

        await query.answer(f"{h}p selected")
        await query.message.edit_caption(
        ...
        asyncio.create_task(
            handle_record(client, session['message'], selection=session)
        )
    except Exception as e:
        LOG.exception("Hotstar quality callback failed: %s", e)
        await query.answer("Quality error.", show_alert=True)

@app.on_callback_query(filters.regex(r"^hotback:"))
async def hotstar_quality_back(client, query):
    try:
        ...
        LOG.exception("Hotstar back failed: %s", e)
...
async def sony_channel_callback(client, query):
    ...
    if not await _capture_live_preview(url, preview_path, provider):
...
@app.on_message(filters.command("Hotstar"))
async def hotstar_command(client, message: Message):
    """Interactive Jio Hotstar recorder: channel -> preview -> audio -> quality -> recording."""
    try:
        await message.delete()
    except Exception:
        pass
    ...
    if len(args) < 2:
        return await message.reply_text(
            "❌ **Invalid Format**\n\n"
            "`/Hotstar <DURATION> <FILENAME>`\n\n"
            "Example: `/Hotstar 00:00:30 StarSports`"
        )
    ...
    if uid is None:
        return await message.reply_text(
            "❌ Unable to identify the recording session."
        )
    ...
    await _safe_callback_answer if False else asyncio.sleep(0)

    try:
        await message.reply_text(
            "🔄 **Loading Hotstar channels...**"
        )
    except Exception:
        pass
    ...
    if not items:
        ...
        return await client.send_message(
            getattr(message.chat, "id", uid),
            "❌ **Hotstar playlist load failed.**\n\n"
            "Please try `/Hotstar` again later."
        )

    session["hotstar_items"] = items

    rows = []

    for index, (title, _url, _group) in enumerate(items[:100]):
        rows.append([
            InlineKeyboardButton(
                f"📺 {title[:48]}",
                callback_data=f"hotstarch:{token}:{index}"
            )
        ])
...
@app.on_callback_query(filters.regex(r"^hotstarch:"))
async def hotstar_channel_callback(client, query):
    parts = query.data.split(":", 2)
    ...
    if not session or not session.get("hotstar_new_command"):
        return await _safe_callback_answer(
            query,
            "Hotstar setup expired. Run /Hotstar again.",
            show_alert=True
        )
    ...
    if index < 0 or index >= len(items):
        return await _safe_callback_answer(
            query,
            "Hotstar channel not found.",
            show_alert=True
        )

    title, url, _group = items[index]

    await _safe_callback_answer(
        query,
        "Detecting Hotstar stream..."
    )

    if not url:
        return await query.message.edit_text(
            "❌ Hotstar channel link not found."
        )

    session["url"] = url
    session["raw_source_name"] = title
    session["selected_video_index"] = None
    session["quality_video_index"] = None

    heights, lang_indexes, selected_video_index, probe_error = \
        await _probe_streams(url)

    async def _hotstar_error(text):
        try:
            return await query.message.edit_text(text)
        except Exception:
            try:
    ...
    if lang_indexes is None:
        ...
        return await _hotstar_error(
            "❌ **Unable to probe Hotstar stream.**" + detail
        )

    detected_audio = {
        lang
        for lang, indexes in lang_indexes.items()
        if indexes
    }
    ...
    preview_dir = join(
        preview_root,
        f"hotstar_preview_{uid}_{secrets.token_hex(4)}"
    )
    ...
    if not await _capture_live_preview(url, preview_path, provider):
        ...
        return await _hotstar_error(
            "❌ **Unable to capture live stream preview.**"
        )

    caption = (
        f"🎬 **Hotstar Stream Preview**\n\n"
        f"📺 **Channel:** `{title}`\n\n"
        "🖼️ Screenshot captured from the live stream.\n\n"
        + _audio_text(session)
    )
...
@app.on_message(filters.command(["rec", "drec"]))
async def rec_command(client, message: Message):
    ...
    command_name = str(raw_text.split(maxsplit=1)[0] if raw_text else "rec").lstrip("/").casefold()
    direct_record_mode = command_name == "drec"
    try:
        raw_args=shlex.split(raw_text)
    except Exception:
        raw_args=raw_text.split()
    if raw_args and str(raw_args[0]).lstrip("/").casefold() in {"rec", "drec"}:
        args=raw_args[1:]
    else:
        args=raw_args
    ...
    if direct_record_mode:
        if len(args) < 2:
            return await message.reply_text("❌ Format: `/drec LINK 00:00:30`")
        if not str(args[0]).strip().lower().startswith(("http://", "https://")):
            return await message.reply_text("❌ `/drec` requires a direct HTTP(S) playlist/stream URL.")
    ...
    if is_playlist_url:
        ...
        if direct_record_mode:
            raw_filename = (" ".join(args[2:]).strip() if len(args) >= 3 else config.DEFAULT_FILENAME)
        else:
            if len(args) < 3:
                return await message.reply_text(
            ...
            if not raw_filename:
                return await message.reply_text("❌ **Filename is required for direct URLs.**")
        url = source_name
    ...
    provider = playlist_name if playlist_name in {"Tata Play", "Airtel", "FW"} else None
    heights, lang_indexes, selected_video_index, probe_error = await _probe_streams(url, provider)
    ...
    session = {
        ...
        "direct_record_mode": direct_record_mode,
    }

    if direct_record_mode:
        # /drec immediately starts the direct-remux recording.
        session["audio"] = set(detected_audio)
        session["watermark"] = "off"
        session["quality"] = "auto"
        session["process_message"] = None
        shutil.rmtree(preview_dir, ignore_errors=True)
        await handle_record(client, message, session)
        return
...
async def handle_record(client, message, selection=None):
    ...
    try:
        ...
        # Fastway (-FW) uses the provider's HLS/M3U8 stream with the
        # connection settings required for reliable direct remux recording.
        # Keep these arguments scoped to FW so existing providers retain their
        # current recording behavior.
        fw_direct_remux = bool(selection.get("direct_record_mode")) or str(selection.get("provider", "")).strip().casefold() == "fw"
        args = [
        ...
        if vf:
            ...
            args += ["-c:v", "copy"]
        if fw_direct_remux:
            # Direct-remux mode (/drec and FW): preserve the source streams.
            # Requested equivalent:
            # -map 0:v:0 -map 0:a? -c:v copy -c:a copy -t <duration>
            # MP4 is used because +faststart is an MP4/MOV muxer option.
            fw_filename = os.path.splitext(filename)[0] + ".mp4"
            video_path = join(save_dir, fw_filename)
            processing_tasks[task_id]["video_path"] = video_path
            user_status[user_id]["output_filename"] = fw_filename
            args = [
                "ffmpeg", "-hide_banner", "-y",
                "-protocol_whitelist", "file,http,https,tcp,tls,crypto",
                "-rw_timeout", "30000000",
        ...
        if wm == "off" and quality == "auto" and not force_16_9 and not fw_direct_remux:
        ...
        final_extension = ".mp4" if fw_direct_remux else ".mkv"
        final_filename = (
            f"{raw_filename.strip()}.[{recording_date_label}]."
            f"[{recording_start_label}-{recording_end_label}]."
            f"{quality_label}.WEB-DL.{audio_type}.UNK.-namebot{final_extension}"
        )
        ...
        caption = (
            ...
            f"📁 **Format:** {'MP4' if fw_direct_remux else 'MKV'}\n"
            f"🆔 **Task ID:** `{task_id}`\n"
            f"🤖 **Bot:** @{_bot_username}\n\n"
        ...
        if save_dir and os.path.exists(save_dir):
            # Keep every successfully uploaded recording on the server for 2 hours.
            # The Telegram copy is already uploaded; later ServerFiles actions use
            # this local copy directly and never download it again from Telegram.
            _register_server_file(
                Path(save_dir),
                owner_id=user_id,
                chat_id=int(getattr(message.chat, "id", 0) or 0),
                filename=os.path.basename(video_path) if video_path else raw_filename,
                kind="recording_partial" if was_cancelled else "recording",
            )
...
async def task_progress_callback(client, query):
    ...
    query_chat_id = int(getattr(getattr(query.message, "chat", None), "id", 0) or 0)
    if (
        owner_id is None
        or (query.from_user.id != owner_id
            and not (query_chat_id in ANONYMOUS_ADMIN_GROUP_IDS and int(state.get("chat_id", 0) or 0) == query_chat_id))
    ):
        return await _safe_callback_answer(
            query,
...
SERVER_FILE_TTL_SECONDS = 2 * 60 * 60
SERVER_FILE_META_NAME = ".server_file.json"
_server_file_cleanup_tasks = set()
...
def _server_files_root() -> Path:
    root = Path(getattr(config, "DOWNLOAD_DIRECTORY", "/tmp"))
    root.mkdir(parents=True, exist_ok=True)
    return root


def _server_file_meta_path(folder: Path) -> Path:
    return folder / SERVER_FILE_META_NAME


def _write_server_file_meta(folder: Path, *, owner_id: int, chat_id: int, filename: str, kind: str = "recording") -> dict:
    now = time.time()
    meta = {
        "owner_id": int(owner_id or 0),
        "chat_id": int(chat_id or 0),
        "filename": str(filename or folder.name),
        "created_at": now,
        "expires_at": now + SERVER_FILE_TTL_SECONDS,
        "kind": kind,
        "folder": folder.name,
    }
    try:
        with _server_file_meta_path(folder).open("w", encoding="utf-8") as fh:
            json.dump(meta, fh, ensure_ascii=False, indent=2)
    except Exception as exc:
        LOG.warning("Unable to write server-file metadata for %s: %s", folder, exc)
    return meta


def _read_server_file_meta(folder: Path) -> Optional[dict]:
    try:
        with _server_file_meta_path(folder).open("r", encoding="utf-8") as fh:
            data = json.load(fh)
...
def _server_file_media(folder: Path) -> Optional[Path]:
    if not folder.is_dir():
        return None
    meta = _read_server_file_meta(folder)
    preferred = str(meta.get("filename", "")) if meta else ""
    if preferred:
        candidate = folder / preferred
        if candidate.is_file() and candidate.name != SERVER_FILE_META_NAME:
            return candidate
    candidates = [x for x in folder.iterdir() if x.is_file() and x.name != SERVER_FILE_META_NAME and x.suffix.lower() in {".mp4", ".mkv", ".m4v", ".ts", ".webm"}]
    if not candidates:
        return None
    return max(candidates, key=lambda x: x.stat().st_size)


def _server_file_folder(token: str) -> Optional[Path]:
    if not token or token in {".", ".."} or "/" in token or "\\" in token:
        return None
    root = _server_files_root().resolve()
...
def _server_file_button_markup(token: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✂️💧🗜️ Trim & Watermark & Compress", callback_data=f"srv:combo:{token}")],
        [InlineKeyboardButton("📸 Screenshot", callback_data=f"srv:screenshot:{token}"), InlineKeyboardButton("☁️ Google Drive", callback_data=f"srv:gdrive:{token}")],
        [InlineKeyboardButton("📐 16:9", callback_data=f"srv:169:{token}")],
        [InlineKeyboardButton("🗑️ Delete", callback_data=f"srv:delete:{token}")],
    ])


async def _delete_server_file_later(folder: Path, expires_at: float):
    try:
        delay = max(0, float(expires_at) - time.time())
        await asyncio.sleep(delay)
        if folder.exists():
            shutil.rmtree(folder, ignore_errors=True)
            LOG.info("Deleted expired server file: %s", folder)
    except asyncio.CancelledError:
        raise
    except Exception as exc:
        LOG.warning("Server-file cleanup failed for %s: %s", folder, exc)


def _schedule_server_file_cleanup(folder: Path, expires_at: float):
    task = asyncio.create_task(_delete_server_file_later(folder, expires_at))
    _server_file_cleanup_tasks.add(task)
    task.add_done_callback(_server_file_cleanup_tasks.discard)
    return task


def _register_server_file(folder: Path, *, owner_id: int, chat_id: int, filename: str, kind: str = "recording") -> dict:
    meta = _write_server_file_meta(folder, owner_id=owner_id, chat_id=chat_id, filename=filename, kind=kind)
    _schedule_server_file_cleanup(folder, meta["expires_at"])
    return meta


def _server_files_for_actor(actor_id: int):
    root = _server_files_root()
    entries = []
    now = time.time()
    for folder in root.iterdir():
        ...
        if owner_id != int(actor_id) and not _is_owner(actor_id):
            continue
        media = _server_file_media(folder)
        if not media:
            continue
        entries.append((folder, meta, media))
    entries.sort(key=lambda item: float(item[1].get("created_at", 0) or 0), reverse=True)
    return entries


def _format_server_expiry(expires_at: float) -> str:
...
async def _process_video_tool(client, message, info, mode, params):
    ...
    server_input = params.get("server_input_path")
    if server_input:
        input_path = Path(str(server_input)).resolve()
        root = _server_files_root().resolve()
        try:
            input_path.relative_to(root)
        except ValueError:
            raise RuntimeError("Invalid server file path.")
        if not input_path.is_file():
            raise RuntimeError("Server-saved file has expired or was deleted.")
    ...
    status = await message.reply_text(_tool_status("⚡ Using server-saved file..." if server_input else "📥 Downloading...", 0))
    try:
        if not server_input:
        ...
        output_markup = None
        if mode in ("169", "169bars"):
            output_markup = InlineKeyboardMarkup([[
                InlineKeyboardButton("✂️💧🗜️ Trim & Watermark & Compress", callback_data=f"vout:combo:{task_dir.name}")
            ]])
        with output_path.open("rb") as fh:
            sent_output = await client.send_video(
                message.chat.id,
                fh,
        ...
        if mode in ("169", "169bars"):
            # Keep this exact output folder for 2 hours so the new button can
            # process the local file without a Telegram re-download.
            _register_server_file(
                task_dir,
                owner_id=uid,
                chat_id=int(getattr(message.chat, "id", 0) or 0),
        ...
        if mode not in ("169", "169bars"):
            shutil.rmtree(task_dir, ignore_errors=True)


@app.on_message(filters.command(["serverfiles", "server_files"]))
async def server_files_command(client, message: Message):
    if not await _verification_required(message):
        return
    uid = _rec_actor_id(message)
    entries = _server_files_for_actor(uid)
    if not entries:
        return await message.reply_text("💾 **Server Saved Files**\n\nNo active server-saved files.\n\nRecord a video first. Files are kept for 2 hours and then deleted automatically.")
    lines = ["💾 **Server Saved Files**\n"]
    for index, (_folder, meta, media) in enumerate(entries, 1):
        size_mb = media.stat().st_size / (1024 * 1024)
        lines.append(
            f"**{index}. {meta.get('filename', media.name)}**\n"
            f"   ⏱️ Expires in: {_format_server_expiry(float(meta.get('expires_at', time.time())))}\n"
            f"   📦 Size: {size_mb:.2f} MB\n"
        )
    # Keep each file's actions directly under its own entry.
    await message.reply_text("\n".join(lines), reply_markup=InlineKeyboardMarkup([
        row
        for folder, _meta, _media in entries
        for row in [
            [InlineKeyboardButton("✂️💧🗜️ Trim & Watermark & Compress", callback_data=f"srv:combo:{folder.name}")],
            [InlineKeyboardButton("📸 Screenshot", callback_data=f"srv:screenshot:{folder.name}"), InlineKeyboardButton("☁️ Google Drive", callback_data=f"srv:gdrive:{folder.name}")],
            [InlineKeyboardButton("📐 16:9", callback_data=f"srv:169:{folder.name}"), InlineKeyboardButton("🗑️ Delete", callback_data=f"srv:delete:{folder.name}")],
        ]
    ]))
...
@app.on_callback_query(filters.regex(r"^vout:combo:[^:]+$"))
async def server_output_combo_callback(client, query):
    if not query.message:
        return await _safe_callback_answer(query, "Message unavailable.", show_alert=True)
    uid = int(query.from_user.id)
    if not _is_owner(uid) and not (bot_settings.get("premium_access", True) and _is_premium(uid)):
        if int(getattr(getattr(query.message, "chat", None), "id", 0) or 0) not in ALLOWED_REC_GROUP_IDS or (bot_settings.get("token", True) and not _has_valid_access(uid)):
            return await _safe_callback_answer(query, "🔒 Token required. Use /Token first.", show_alert=True)
    token = query.data.split(":", 2)[2]
    folder = _server_file_folder(token)
    media = _server_file_media(folder) if folder else None
    if not folder or not media:
        return await _safe_callback_answer(query, "⏱️ Server file expired or was deleted.", show_alert=True)
    meta = _read_server_file_meta(folder) or {}
    owner_id = int(meta.get("owner_id", 0) or 0)
    if owner_id != uid and not _is_owner(uid):
        return await _safe_callback_answer(query, "❌ This server file belongs to another user.", show_alert=True)
    info = {"file_id": "", "file_name": media.name, "file_size": media.stat().st_size}
    key = (uid, int(query.message.chat.id), int(query.message.id))
    VIDEO_TOOL_STATES[key] = {"mode": "trim_watermark_compress", "info": info, "original_id": int(query.message.id), "server_input_path": str(media)}
    await _safe_callback_answer(query, "Server file selected.")
    prompt = await query.message.reply_text(
        "✂️💧🗜️ **Trim & Watermark & Compress**\n\n"
        "⚠️ **This is not a new file.**\n"
        "This is an old file that you previously uploaded.\n"
        "The same server file will be processed directly. Telegram will not download it again.\n"
        "If you do not want to process this file, please click the Cancel button.\n\n"
        "Reply with the settings below. Trim and file name are required; watermark time ranges are optional and relative to the trimmed video.\n\n"
        "`Trim: 00:00:40 to 00:02:00\n"
        "Watermark: 1\n"
        "Watermark Time: 01:00–02:00, 30:00–31:00\n"
        "Watermark Text/Link: (optional)\n"
        ...
        "Target MB: 300`\n\n"
        "Watermark 1 uses the supplied image at x=20, bottom=110, width=300.\n"
        "Use `Watermark: 2` for the alternate bottom=899 position. Leave Watermark Text/Link empty to use the supplied image.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="srv:cancel")]])
    )
    VIDEO_TOOL_STATES[key]["prompt_id"] = int(prompt.id)


@app.on_callback_query(filters.regex(r"^srv:(combo|screenshot|gdrive|169|delete):[^:]+$"))
async def server_files_action_callback(client, query):
    if not query.message:
        return await _safe_callback_answer(query, "Message unavailable.", show_alert=True)
    uid = int(query.from_user.id)
    if not _is_owner(uid) and not (bot_settings.get("premium_access", True) and _is_premium(uid)):
    ...
    if action == "delete":
        shutil.rmtree(folder, ignore_errors=True)
        await _safe_callback_answer(query, "🗑️ Server file deleted.")
        try:
            await query.message.edit_text("✅ Server file deleted.")
        except Exception:
            pass
        return
    info = {"file_id": "", "file_name": media.name, "file_size": media.stat().st_size}
    ...
    if action == "combo":
        ...
        prompt = await query.message.reply_text(
            "✂️💧🗜️ **Trim & Watermark & Compress**\n\n"
            "⚠️ **This is not a new file.**\nThis is an old file that you previously uploaded.\n"
            "Would you like to process this same file as the output file?\n"
            "If not, please click the Cancel button.\n\n"
            "Reply with the settings below. Trim and file name are required; watermark time ranges are optional and relative to the trimmed video.\n\n"
            "`Trim: 00:00:40 to 00:02:00\nWatermark: 1\nWatermark Time: 01:00–02:00, 30:00–31:00\nWatermark Text/Link: (optional)\nFile Name: Anime Cartoon\nTarget MB: 300`\n\n"
            "Watermark 1 uses the supplied image at x=20, bottom=110, width=300.\n"
            "Use `Watermark: 2` for the alternate bottom=899 position. Leave Watermark Text/Link empty to use the supplied image.",
        ...
        VIDEO_TOOL_STATES[key] = {"mode": "screenshot", "info": info, "original_id": int(query.message.id), "server_input_path": str(media)}
        prompt = await query.message.reply_text("📸 **Screenshot**\n\nReply with the number of screenshots.\n\nExample: `10`")
    elif action == "169":
        VIDEO_TOOL_STATES[key] = {"mode": "169", "info": info, "original_id": int(query.message.id), "server_input_path": str(media)}
        prompt = await query.message.reply_text("📐 **16:9**\n\nReply with the new filename.\n\nExample: `Anime Cartoon 16x9`")
    elif action == "gdrive":
        # Reuse the existing Drive workflow through a local server-file state.
        VIDEO_TOOL_STATES[key] = {"mode": "server_gdrive", "info": info, "original_id": int(query.message.id), "server_input_path": str(media)}
        await _safe_callback_answer(query, "Using server-saved file for Google Drive.")
        status = await query.message.reply_text("☁️ **Google Drive Upload**\n\n📄 File: `" + media.name + "`\n🚀 Starting Drive upload...")
        try:
            await upload_and_notify(client, int(query.message.chat.id), str(media), media.name, status_msg=status)
        except Exception as exc:
            await status.edit_text(f"⚠️ **Google Drive upload failed**\n\n`{str(exc)[:1500]}`")
        return
    else:
...
@app.on_callback_query(filters.regex(r"^srv:cancel$"))
async def server_file_cancel_callback(client, query):
    uid = int(query.from_user.id)
    for candidate_key, candidate_state in list(VIDEO_TOOL_STATES.items()):
        if candidate_key[0] == uid and int(candidate_state.get("prompt_id", 0) or 0) == int(getattr(query.message, "id", 0) or 0):
            VIDEO_TOOL_STATES.pop(candidate_key, None)
    await _safe_callback_answer(query, "Cancelled.")
    try:
        await query.message.delete()
    except Exception:
...
async def video_tools_text_input(client, message: Message):
    if not message.reply_to_message:
        return
    if not message.from_user and int(getattr(message.chat, "id", 0) or 0) not in ANONYMOUS_ADMIN_GROUP_IDS:
        return
    ...
    if mode == "screenshot":
        ...
        VIDEO_TOOL_STATES.pop(key, None)
        task_dir = Path(getattr(config, "DOWNLOAD_DIRECTORY", "/tmp")) / f"screenshots_{uid}_{secrets.token_hex(6)}"
        task_dir.mkdir(parents=True, exist_ok=True)
        server_input = state.get("server_input_path")
        if server_input:
            input_path = Path(str(server_input)).resolve()
            root = _server_files_root().resolve()
            try:
...
async def _server_file_cleanup_loop():
    while True:
        try:
            root = _server_files_root()
            now = time.time()
            ...
            LOG.warning("Server-file cleanup loop failed: %s", exc)
...
async def _bot_main():
    ...
    server_cleanup_task = asyncio.create_task(_server_file_cleanup_loop())
    try:
        await idle()
    finally:
        server_cleanup_task.cancel()
