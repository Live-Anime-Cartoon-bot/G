...
def get_watermark_size() -> int:
    """Return saved watermark logo width in pixels, default 80."""
    try:
...
app = Client(
    ...
    max_concurrent_transmissions=4,   # parallel upload chunks
)
...
async def progress_for_pyrogram(current, total, message, start, msg,
    ...
    task_id  = _upload_task_id(id(msg))
...
• `/tpstatus` — Login status check
• `/tpchannels [search]` — Channel list browse karo
• `/tataplayrec` — Tata Play live record wizard
• `/dl -Tplay -c ChannelName -t DD-MM-YYYY HH:MM AM/PM - HH:MM AM/PM -n File`
Download catchup recording
...
**Tata Play Login (Owner Only)**

• `/tplogin <SID>`
Start Tata Play login (SID = Subscriber ID).

• `/tpotp <6-digit code>`
Submit OTP to complete Tata Play login.

• `/tpstatus`
Check Tata Play login status.

• `/tpchannels [search]`
Browse Tata Play channel list.
"""
...
async def _group_gate(client: Client, message: Message):
    ...
    # Public mode: AUTH_USERS not configured → everyone can use private chat
    if not AUTH_USERS:
        return
    ...
    # Unauthorized users — show restriction message with Join Group button
    join_link = GROUP_INVITE_LINK or "https://t.me/+ww77CDQwoigzYjk1"
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("👥 Join Group", url=join_link)
    ]])
    await message.reply_text(
        "❌ The owner of this bot has restricted access.\n"
...
async def watermark_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if wm_img:
            ...
            wm_label = "Image (bottom-right, last 2 min)"
        else:
            ...
            wm_label = f"`{wm_text}` (text, last 2 min)"
        ...
        _dest = await _await_upload_choice(user_id, status, f"Watermarked: `{out_mb:.1f} MB`")
        if _dest != "cancel":
            await _run_upload_destination(
...
async def my_premium_cmd(_client: Client, message: Message):
    ...
    if uid in OWNER_IDS:
        return await message.reply_text(
            ...
            "JioTV / TataPlay: Unlimited recording"
        )
    if is_admin(uid):
        return await message.reply_text(
    ...
    if not entry:
        return await message.reply_text(
            ...
            "• TataPlay recording: max 10 seconds\n\n"
    ...
    if active:
        lines += [
            ...
            "• TataPlay recording: ✅ Unlimited",
        ]
    else:
        lines += [
            "• JioTV recording: ⏱ max 10 seconds (free limit)",
            "• TataPlay recording: ⏱ max 10 seconds (free limit)",
...
# ---------------------------------------------------------------------------
# TataPlay — Login + Channel List + Live Recording (via bot/tataplay.py module)
# ---------------------------------------------------------------------------

_tataplay_cat_cache: dict = {}
_tataplay_ch_cache:  dict = {}


# /tataplaylogin <sid>  — ADMIN ONLY
@app.on_message(filters.command(["tataplaylogin", "TataPlayLogin", "tplogin"]) & _OWNER_FILTER)
async def tataplay_login_cmd(client: Client, message: Message):
    args = message.command
    if len(args) < 2:
        return await message.reply_text(
            "**Usage:** `/tataplaylogin <10-digit Subscriber ID (SID)>`\n\n"
            "Example: `/tataplaylogin 1234567890`\n\n"
            "⚠️ SID = TataPlay subscriber ID, phone number nahi.\n"
            "App ke Settings > Account mein milega."
        )
    sid = args[1].strip()
    if not sid.isdigit() or len(sid) != 10:
        return await message.reply_text("❌ SID exactly 10 digits ka hona chahiye.")

    status = await message.reply_text(f"📲 TataPlay OTP bhej raha hoon SID **{sid}** ke liye…")
    result = await asyncio.get_event_loop().run_in_executor(None, tataplay.send_otp, sid)
    if result["success"]:
        await status.edit_text(
            f"✅ {result['message']}\n\n"
            "OTP mila? Send karo:\n`/tataplayotp <6-digit OTP>`"
        )
    else:
        await status.edit_text(f"❌ {result['message']}")


# /tataplayotp <otp>  — ADMIN ONLY
@app.on_message(filters.command(["tataplayotp", "TataPlayOTP", "tpotp"]) & _OWNER_FILTER)
async def tataplay_otp_cmd(client: Client, message: Message):
    args = message.command
    if len(args) < 2:
        return await message.reply_text("**Usage:** `/tataplayotp <6-digit OTP>`")
    otp    = args[1].strip()
    status = await message.reply_text("🔐 OTP verify kar raha hoon…")
    result = await asyncio.get_event_loop().run_in_executor(None, tataplay.verify_otp, otp)
    if result["success"]:
        await status.edit_text(
            f"✅ **TataPlay Login Successful!**\n\n{result['message']}\n\n"
            "Commands:\n"
            "• `/tataplaychannels` — channel list\n"
            "• `/tataplayrec <channel> <duration>` — live record\n\n"
            "Example: `/tataplayrec Pogo 01:00:00`"
        )
    else:
        await status.edit_text(f"❌ {result['message']}")
...
# /tataplaychannels
@app.on_message(filters.command(["tataplaychannels", "TataPlayChannels", "tpchannels"]) & AUTH)
async def tataplay_channels_cmd(client: Client, message: Message):
    uid = message.from_user.id
    if not tataplay.is_logged_in():
        return await message.reply_text(
            "❌ TataPlay mein login nahi ho.\nPehle: `/tataplaylogin <SID>`"
        )
    status   = await message.reply_text("📡 TataPlay channels fetch kar raha hoon…")
    channels = await asyncio.get_event_loop().run_in_executor(None, tataplay.get_channels)
    if not channels:
        return await status.edit_text(
            "❌ Channels load nahi hue.\nDobara login karo: `/tataplaylogin <SID>`"
        )

    cats: dict = {}
    for ch in channels:
        cat = ch.get("primaryGenre") or ch.get("genre") or ch.get("category") or "Other"
        if isinstance(cat, list):
            cat = cat[0] if cat else "Other"
        cats.setdefault(cat, []).append(ch)

    _tataplay_cat_cache[uid] = cats
    cat_names = sorted(cats.keys())
    rows, row = [], []
    for cat in cat_names[:24]:
        count = len(cats[cat])
        row.append(InlineKeyboardButton(f"{cat} ({count})", callback_data=f"tpcat:{uid}:{cat[:20]}"))
        if len(row) == 2:
            rows.append(row); row = []
    if row:
        rows.append(row)
    ...
    await status.edit_text(
        f"📺 **TataPlay — {len(channels)} Channels**\n\nCategory select karo:",
        reply_markup=InlineKeyboardMarkup(rows)
    )


@app.on_callback_query(filters.regex(r"^tpcat:"))
async def tataplay_cat_cb(client: Client, cq: CallbackQuery):
    parts    = cq.data.split(":", 2)
    uid, cat = int(parts[1]), parts[2]
    if cq.from_user.id != uid:
        return await cq.answer("Yeh tumhara session nahi.", show_alert=True)
    cats     = _tataplay_cat_cache.get(uid, {})
    full_cat = next((k for k in cats if k[:20] == cat), cat)
    channels = cats.get(full_cat, [])
    if not channels:
        return await cq.answer("No channels.", show_alert=True)
    ch_map = _tataplay_ch_cache.setdefault(uid, {})
    for ch in channels:
        name = ch.get("name") or ch.get("channelName") or ch.get("title") or ""
        ch_map[name.lower()] = ch
    rows, row = [], []
    for ch in channels[:48]:
        name  = ch.get("name") or ch.get("channelName") or ch.get("title") or "?"
        ch_id = str(ch.get("id") or ch.get("channelId") or ch.get("channel_id") or "")
        row.append(InlineKeyboardButton(name, callback_data=f"tpch:{uid}:{ch_id}"))
        if len(row) == 2:
            rows.append(row); row = []
    if row:
        rows.append(row)
    rows.append([InlineKeyboardButton("◀️ Back", callback_data=f"tpback:{uid}")])
    await cq.message.edit_text(
        f"📺 **TataPlay — {full_cat}** ({len(channels)} channels)\n\nRecord: `/tataplayrec <name> <duration>`",
        reply_markup=InlineKeyboardMarkup(rows)
    )
    await cq.answer()


@app.on_callback_query(filters.regex(r"^tpback:"))
async def tataplay_back_cb(client: Client, cq: CallbackQuery):
    uid  = int(cq.data.split(":")[1])
    if cq.from_user.id != uid:
        return await cq.answer("Yeh tumhara session nahi.", show_alert=True)
    cats      = _tataplay_cat_cache.get(uid, {})
    ...
    if row:
        rows.append(row)
    await cq.message.edit_text(
        "📺 **TataPlay — Category select karo:**",
        reply_markup=InlineKeyboardMarkup(rows)
    )
    await cq.answer()


@app.on_callback_query(filters.regex(r"^tpch:"))
async def tataplay_ch_cb(client: Client, cq: CallbackQuery):
    parts      = cq.data.split(":", 2)
    uid, ch_id = int(parts[1]), parts[2]
    if cq.from_user.id != uid:
        return await cq.answer("Yeh tumhara session nahi.", show_alert=True)
    ch_map   = _tataplay_ch_cache.get(uid, {})
    ch_entry = next((v for v in ch_map.values()
                     if str(v.get("id") or v.get("channelId") or v.get("channel_id") or "") == ch_id), None)
    ch_name  = ((ch_entry.get("name") or ch_entry.get("channelName") or ch_id) if ch_entry else ch_id)
    await cq.answer(f"✅ {ch_name} selected!")
    await cq.message.reply_text(
        f"📺 **{ch_name}** select hua!\n\nRecord karne ke liye:\n`/tataplayrec {ch_name} 01:00:00`"
    )


# /tataplayrec <channel name> <duration>
@app.on_message(filters.command(["tataplayrec", "TataPlayRec"]) & AUTH)
async def tataplay_rec_cmd(client: Client, message: Message):
    uid = message.from_user.id
    try:
        if not tataplay.is_logged_in():
            return await message.reply_text(
                "❌ **TataPlay mein login nahi hai.**\n\n"
                "Admin se kaho pehle yeh kare:\n"
                "`/tataplaylogin <SID>`\n\n"
                "SID TataPlay account se milta hai."
            )
        args = message.command
        if len(args) < 3:
            return await message.reply_text(
                "**Usage:** `/tataplayrec <channel name> <duration>`\n\n"
                "Example: `/tataplayrec Pogo 01:00:00`\n"
                "Channel list: `/tataplaychannels`"
            )
        duration_str = args[-1].strip()
        ch_name_raw  = " ".join(args[1:-1]).strip()
        duration_sec = _parse_duration_token(duration_str)
        ...
        status   = await message.reply_text(f"🔍 TataPlay mein **{ch_name_raw}** dhundh raha hoon…")
        channels = await asyncio.get_event_loop().run_in_executor(None, tataplay.get_channels)

        ch_name_lower = ch_name_raw.lower()
        matched = None
        for ch in channels:
            name = (ch.get("name") or ch.get("channelName") or ch.get("title") or "").lower()
            if name == ch_name_lower:
                matched = ch; break
        if not matched:
            for ch in channels:
        ...
        if not matched:
            similar = [(ch.get("name") or ch.get("channelName") or "")
                       for ch in channels
                       if ch_name_lower[:4] in (ch.get("name") or ch.get("channelName") or "").lower()][:6]
            hint = "\n".join(f"• {s}" for s in similar) if similar else "Channel list: `/tataplaychannels`"
            return await status.edit_text(f"❌ Channel **{ch_name_raw}** nahi mila.\n\nShayad:\n{hint}")

        ch_id    = str(matched.get("id") or matched.get("channelId") or matched.get("channel_id") or "")
        ch_label = matched.get("name") or matched.get("channelName") or ch_name_raw

        await status.edit_text(f"📡 **{ch_label}** ka stream URL fetch kar raha hoon…")
        result = await asyncio.get_event_loop().run_in_executor(None, tataplay.get_stream_url, ch_id)
        if not result["success"]:
            return await status.edit_text(
                f"❌ Stream URL nahi mila: {result['message']}\n\nDobara login karo: `/tataplaylogin <SID>`"
            )
        ...
        setup = {
            ...
            "effective_url": stream_url, "orig_msg": message,
            "is_hls": True,
            "tataplay": True, "tataplay_channel": ch_label,
        }
        rec_setup_sessions[uid] = setup
        await status.edit_text(
            f"✅ **{ch_label}** — Stream ready!\n"
            f"⏱ Duration: `{dur_hms}`\n📡 Source: TataPlay Live\n\n⚙️ Setup wizard khul raha hai…"
        )
        tracks = await _probe_audio_tracks(stream_url)
        setup["detected_audio_tracks"] = tracks
        wizard_msg = await message.reply_text(_audio_step_text(setup), reply_markup=_kb_audio_step(setup))
        ...
        LOG.exception("tataplay_rec_cmd error uid=%s: %s", uid, e)
        try:
            await message.reply_text(
                f"❌ **Kuch galat ho gaya:**\n`{e}`\n\n"
                f"Agar yeh baar baar aaye toh `/tataplaylogin` se dobara login karo."
            )
        except Exception:
            pass


# /tataplaylogout  — ADMIN ONLY
@app.on_message(filters.command(["tataplaylogout", "TataPlayLogout"]) & _OWNER_FILTER)
async def tataplay_logout_cmd(client: Client, message: Message):
    if not tataplay.is_logged_in():
        return await message.reply_text("❌ TataPlay mein login nahi hai.")
    tataplay.logout()
    await message.reply_text("✅ TataPlay logout ho gaya. Ab koi bhi TataPlay use nahi kar sakta.")


# /tataplaystatus
@app.on_message(filters.command(["tataplaystatus", "TataPlayStatus", "tpstatus"]) & AUTH)
async def tataplay_status_cmd(client: Client, message: Message):
    if not tataplay.is_logged_in():
        return await message.reply_text("❌ TataPlay login nahi hai.\nUse: `/tataplaylogin <SID>`")
    await message.reply_text(
        "✅ **TataPlay Login Active**\n\n"
        "Commands:\n"
        "• `/tataplaychannels` — channel list\n"
        "• `/tataplayrec <channel> <duration>` — live record\n"
        "• `/tataplaylogout` — logout"
    )


# ---------------------------------------------------------------------------
# /dl  — JioTV / TataPlay catchup download
# Syntax:
#   /dl -Jiotv -c ChannelName -t DD-MM-YYYY HH:MM AM/PM - HH:MM AM/PM -n File
#   /dl -Tplay -c ChannelName -t DD-MM-YYYY HH:MM AM/PM - HH:MM AM/PM -n File
# ---------------------------------------------------------------------------
...
def _parse_dl_command(text: str) -> dict | None:
    ...
    src_match = _re.search(r"-([Jj]iotv|[Tt]play|[Tt]ataplay)\b", text)
    if not src_match:
        return None
    source = "jiotv" if "jio" in src_match.group(1).lower() else "tplay"
...
async def dl_catchup_cmd(client: Client, message: Message):
    """
    /dl -Jiotv/-Tplay -c Channel -t DD-MM-YYYY HH:MM AM/PM - HH:MM AM/PM -n Filename
    Downloads a TV catchup recording and sends it.
    """
    uid = message.from_user.id
    status = None
    ...
    if not parsed:
        return await message.reply_text(
            ...
            "`/dl -Tplay -c Sony LIV -t 01-07-2026 08:00 PM - 10:00 PM -n SonyLIV_Show`"
        )
    ...
    src_label = "JioTV" if source == "jiotv" else "Tata Play"
    ...
    if source == "jiotv":
        ...
        if not tataplay.is_logged_in():
            return await status.edit_text(
                "❌ TataPlay login nahi hai.\n"
                "Admin se kaho: `/tataplaylogin <SID>`"
            )
        matches = await asyncio.get_event_loop().run_in_executor(
            None, tataplay.search_channel, ch_name
        )

    if not matches:
        return await status.edit_text(
            f"❌ Channel **{ch_name}** nahi mila.\n\n"
            f"Check: `/{'channels' if source == 'jiotv' else 'tpchannels'}` se list dekho."
        )
    ...
    if source == "jiotv":
        ...
        result = await asyncio.get_event_loop().run_in_executor(
            None, tataplay.get_catchup_url, ch_id, begin_ts, end_ts
        )
...
async def dl_help_cmd(client: Client, message: Message):
    await message.reply_text(
        ...
        "`/dl -Tplay -c Sony Entertainment -t 30-06-2026 07:00 PM - 09:00 PM -n SonyEnt`\n\n"
        "**Notes:**\n"
        ...
        "• Use `/tpchannels` to search TataPlay channels"
    )
