...
def _auth_filter():
    ...
    If a GROUP_CHAT_ID is configured, every message from that group is
    allowed through the common AUTH gate, including anonymous/admin
    messages where ``message.from_user`` may be unavailable.
    Outside the configured group, only AUTH_USERS and OWNER_IDS are allowed.

    The group check is evaluated at message time so the existing /Group
    command can update GROUP_CHAT_ID without restarting the bot.
    """
    if not AUTH_USERS:
        return filters.all

    user_filter = filters.user(AUTH_USERS) | filters.user(OWNER_IDS or [])

    async def _group_or_user(_, client, message):
        if GROUP_CHAT_ID and message.chat and message.chat.id == GROUP_CHAT_ID:
            return True
        return bool(message.from_user and (
            message.from_user.id in AUTH_USERS or
            message.from_user.id in (OWNER_IDS or [])
        ))

    return filters.create(_group_or_user)
...
async def start_cmd(client: Client, message: Message):
    # In a configured group, /start must also work for anonymous/admin
    # messages where Telegram may not provide message.from_user.
