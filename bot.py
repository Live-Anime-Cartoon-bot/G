            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) >= 1024:
                ...
                try:
                    ...
                    try:
                        ...
                        partial_uploaded = False
            partial_sent_text = "⚠️ Partial Recording Sent\n\n" if partial_uploaded else ""
            upload_result_text = (
                "📤 The recorded portion has been uploaded successfully.\n\n"
                if partial_uploaded
                else "⚠️ No partial recording was available to upload.\n\n"
            )
            await status_message.edit_text(
                ...
                f"{upload_result_text}"
                "⏳ Server copy auto-deletes in 1 hour.",
                parse_mode=ParseMode.MARKDOWN,
            )
            return
...
def _parse_trim_range(args: list[str], duration: float) -> tuple[int, int]:
    """Parse /trim START to END safely.

    Accepts HH:MM:SS, MM:SS, shorthand (30s/2m/1h), and separators
    such as `to`, `until`, or `-`. Separator words are never parsed as
    timestamps.
    """
    raw = " ".join(str(value).strip() for value in args if str(value).strip())
    if not raw:
        raise ValueError("Usage: /trim START to END")

    # Normalize common separators so a later parser only sees two values.
    normalized = re.sub(r"\b(?:to|until)\b", " ", raw, flags=re.IGNORECASE)
