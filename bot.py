...
async def blackbars_callback(client: Client, cq: CallbackQuery):
    ...
    if action == "new":
        ...
        state["wm_start"] = "00:10:00"
        state["wm_end"] = "00:11:00"
    ...
    start_sec = _blackbars_hms_seconds(state["wm_start"])
    end_sec = _blackbars_hms_seconds(state["wm_end"])

    if end_sec <= start_sec:
        return await cq.answer(
            "Watermark End time Start time se baad hona chahiye.",
            show_alert=True,
        )
    ...
    try:
        await status.edit_text(
            ...
            f"Time: `{state['wm_start']}` → `{state['wm_end']}`"
        )
        ...
        if start_sec >= duration:
            raise Exception(
                f"Watermark start `{state['wm_start']}` is past the video end "
                f"`{_seconds_to_hms(int(duration))}`."
            )

        # Clamp the watermark interval to the actual video duration.
        ...
        # Scale the logo according to the requested button and show it only
        # between Start and End.
        fc = (
            f"[0:v]{base_vf}[base];"
            f"[1:v]scale={wm['scale']}:-1[watermark];"
            f"[base][watermark]overlay={wm['xy']}:"
            f"enable='between(t,{start_sec},{end_sec})'[outv]"
        )
        ...
        caption = (
            ...
            f"Aspect: `{aspect_label}`\n"
            f"Watermark: `{wm['label']}`\n"
            f"Watermark Time: `{state['wm_start']}` → `{state['wm_end']}`\n"
            f"Size: `{out_size_mb:.1f} MB`\n"
