...
# ---------------------------------------------------------------------------
# /BlackBars — process a replied video with quality/aspect + timed watermark
# ---------------------------------------------------------------------------
...
_BLACKBARS_WM_POS = {
    # Exact positions requested for the two new watermark buttons.
    "top_left": {
        "label": "Top-Left",
        "scale": 140,
        "xy": "(main_w-overlay_w)/2-10:62",
    },
    "left_corner": {
        "label": "Left Corner",
        "scale": 240,
        "xy": "20:main_h-overlay_h-60",
    },
}
...
def _blackbars_keyboard(state: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [abtn("crop169")],
        [InlineKeyboardButton("💧 Watermark", callback_data=f"bb:{uid}:noop")],
...
def _blackbars_text(state: dict) -> str:
    ...
    return (
        ...
        "**Watermark**\n"
        f"Position: `{wm}`\n"
        "Watermark Timing: `Automatic by video duration`\n"
        "• `< 00:00:10` → None\n"
        "• `≥ 00:00:10` → 00:00:10 → 00:00:30\n"
        "• `≥ 00:10:00` → + 00:10:00 → 00:11:00\n"
        "• `≥ 00:30:00` → + 00:30:00 → 00:31:00\n\n"
...
def _blackbars_filter(state: dict) -> str:
    ...
    if aspect == "bars169":
        ...
        # 4:5 canvas; quality is represented by the canvas height.
        ...
        # 16:9 crop, then scale to the selected output quality.
        vf = (
...
def _blackbars_watermark_windows(duration: float) -> list[tuple[int, int]]:
    """Return automatic watermark windows based on the source video duration."""
    d = float(duration or 0)
    if d < 10:
        return []
    windows = [(10, 30)]
    if d >= 600:
        windows.append((600, 660))
    if d >= 1800:
        windows.append((1800, 1860))
    return windows


def _blackbars_watermark_expr(duration: float) -> str:
    windows = _blackbars_watermark_windows(duration)
    if not windows:
        return "0"
    return "+".join(f"between(t,{start},{end})" for start, end in windows)


def _blackbars_watermark_label(duration: float) -> str:
    windows = _blackbars_watermark_windows(duration)
    if not windows:
        return "None (video < 00:00:10)"
    return " + ".join(
        f"{_seconds_to_hms(start)} → {_seconds_to_hms(end)}"
...
async def blackbars_cmd(client: Client, message: Message):
    ...
    if not src_media:
        return await message.reply_text(
            ...
            "then select the watermark position."
        )

    # Default values exactly matching the requested watermark timing.
    state = {
        "user_id": user_id,
        "src_msg": src_msg,
        "quality": "720",
...
@app.on_callback_query(filters.regex(
    r"^bb:(\d+):(q|a|p|new|start|cancel|noop)(?::([a-z0-9_]+))?$"
))
async def blackbars_callback(client: Client, cq: CallbackQuery):
    ...
    if action == "noop":
        return await cq.answer("Watermark settings neeche hain.")
    ...
    if action == "new":
        state["quality"] = "720"
        state["aspect"] = "bars169"
        state["wm_pos"] = "top_left"
        state["wm_start"] = "00:00:10"
        state["wm_end"] = "00:00:30"
    ...
    # Watermark timing is automatic from the source duration:
    # <10s: none; >=10m: first+second; >=30m: first+second+third.

    blackbars_sessions.pop(uid, None)
    src_msg = state["src_msg"]
    q = state["quality"]
    aspect_label = _BLACKBARS_ASPECTS[state["aspect"]]
    wm = _BLACKBARS_WM_POS[state["wm_pos"]]

    await cq.answer("Starting…")

    save_dir = join(DOWNLOAD_DIRECTORY, f"blackbars_{int(time.time())}")
    os.makedirs(save_dir, exist_ok=True)
    status = cq.message
    user_tasks[uid] = True
    success = False
    ...
    try:
        await status.edit_text(
            ...
            f"Watermark: `{wm['label']}`\n"
            f"Watermark: `{_blackbars_watermark_label(duration) if 'duration' in locals() else 'Auto by video duration'}`"
        )
        ...
        base_vf = _blackbars_filter(state)
        wm_windows = _blackbars_watermark_windows(duration)
        wm_expr = _blackbars_watermark_expr(duration)

        if not wm_windows:
            # Videos shorter than 10 seconds must not receive a watermark.
            fc = f"[0:v]{base_vf}[outv]"
        else:
            # Requested watermark image.
            ...
            if not wm_img:
                raise Exception(
                    "Watermark image could not be downloaded. "
                    "Please check the configured watermark image URL."
                )

            # Same watermark is automatically shown in every applicable window.
            fc = (
                f"[0:v]{base_vf}[base];"
                f"[1:v]scale={wm['scale']}:-1[watermark];"
                f"[base][watermark]overlay={wm['xy']}:"
                f"enable='{wm_expr}'[outv]"
            )

        src_name = os.path.splitext(
            os.path.basename(getattr(src_media, "file_name", "") or "video")
        )[0]
        if not src_name or src_name == "video":
            src_name = "video"

        out_path = join(
            save_dir,
            f"{src_name}_BlackBars_{q}p.mp4"
        )

        await status.edit_text(
            "🎬 **Processing /BlackBars…**\n\n"
            f"Quality: `{q}p`\n"
            f"Aspect: `{aspect_label}`\n"
            f"Watermark: `{wm['label']}`\n"
            f"Times: `{_blackbars_watermark_label(duration)}`\n\n"
            "Re-encoding with FFmpeg…"
        )

        wm_input = f'-i {shlex.quote(wm_img)} ' if wm_windows else ''
        ...
        if (
            ...
            raise Exception(
                f"FFmpeg /BlackBars failed:\n{(err or 'Unknown error')[-1800:]}"
            )
        ...
        caption = (
            ...
            f"Aspect: `{aspect_label}`\n"
            f"Watermark: `{wm['label']}`\n"
            f"Watermark Times: `{_blackbars_watermark_label(duration)}`\n"
            f"Size: `{out_size_mb:.1f} MB`\n"
        ...
        _dest = await _await_upload_choice(
            uid, status, f"BlackBars: `{out_size_mb:.1f} MB`"
        )
        if _dest != "cancel":
            await _run_upload_destination(
                client, uid, _dest, status, out_path, caption, duration,
                thumb_path=thumb_path, save_dir=save_dir,
            )
        ...
        LOG.error(f"/BlackBars failed uid={uid}: {e}", exc_info=True)
        err_text = str(e)
        if len(err_text) > 2500:
            err_text = "...[truncated]...\n" + err_text[-2500:]
        try:
