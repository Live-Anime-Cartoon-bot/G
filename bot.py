...
def _admin_auth_filter(_filter, _client, message):
    """Allow configured admins through every handler using AUTH."""
    uid = getattr(getattr(message, "from_user", None), "id", 0)
    if uid and (is_owner(uid) or is_admin(uid)):
        return True

    # Telegram anonymous-admin messages can be attributed to the group chat
    # instead of a human user.  When Anonymous Admin is enabled, only the
    # configured bot group is accepted through this path.
    if (is_anonymous_admin_enabled() and GROUP_CHAT_ID and
            getattr(getattr(message, "sender_chat", None), "id", None) == GROUP_CHAT_ID):
        return True
    return False
...
@app.on_message(filters.command(["googledrive", "gdrive"]))
...
@app.on_message(filters.command(["Drivelogout", "drivelogout", "gdrive_logout", "drivelog"]))
