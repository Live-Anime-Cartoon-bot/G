...
# ---------------------------------------------------------------------------
# /BlackBars — replied-video resize/padding with independently timed watermark
# ---------------------------------------------------------------------------

# BlackBars jobs are intentionally separate from the existing ffmpeg_jobs and
# watermark implementation so cancellation/progress for one user cannot touch
# another user's task or an existing command.
blackbars_jobs: dict = {}
blackbars_sessions: dict = {}
...
def _blackbars_task_id(uid: int) -> str:
    # Include enough entropy to avoid collisions when one user starts jobs close
    # together, while keeping callback_data comfortably short.
    return hashlib.sha1(f"blackbars:{uid}:{time.time_ns()}:{pysecrets.token_hex(4)}".encode()).hexdigest()[:12]


def _blackbars_parse_time(value: str) -> int | None:
...
def _blackbars_parse_timing(text: str) -> tuple[int, int] | None:
    """Accept Start HH:MM:SS End HH:MM:SS in flexible whitespace/case."""
    m = re.search(
        r"start\s+([0-9]{1,2}:[0-9]{2}:[0-9]{2})\s+end\s+([0-9]{1,2}:[0-9]{2}:[0-9]{2})",
        text or "",
        flags=re.IGNORECASE,
    )
    if not m:
        # Also accept a simple HH:MM:SS-HH:MM:SS form for convenience.
        m = re.search(
            r"([0-9]{1,2}:[0-9]{2}:[0-9]{2})\s*(?:-|to)\s*([0-9]{1,2}:[0-9]{2}:[0-9]{2})",
            text or "",
            flags=re.IGNORECASE,
        )
    if not m:
        return None
    start = _blackbars_parse_time(m.group(1))
    end = _blackbars_parse_time(m.group(2))
    if start is None or end is None:
        return None
    return start, end


def _blackbars_validate_timing(ranges: list[tuple[int, int]], duration: float) -> str | None:
    """Return an error string, or None when all ranges are valid/non-overlapping."""
    normalized = []
    for start, end in ranges:
        ...
        if end > duration + 0.001:
            return f"End time cannot exceed the video duration ({_ff_sec_to_hms(duration)})."
        normalized.append((start, end))
    normalized.sort()
    for previous, current in zip(normalized, normalized[1:]):
        if current[0] < previous[1]:
            return "Watermark timing ranges must not overlap."
    return None


def _blackbars_timing_text(ranges: list[tuple[int, int]]) -> str:
    if not ranges:
        return "_No watermark timings added yet._"
    return "\n".join(
        f"Start `{_ff_sec_to_hms(start)}`\nEnd   `{_ff_sec_to_hms(end)}`"
        for start, end in ranges
    )


def _blackbars_setup_text(state: dict) -> str:
    mode_labels = {
    ...
    return (
        ...
        f"Mode: `{mode_labels.get(state['mode'], state['mode'])}`\n\n"
        f"{_blackbars_timing_text(state.get('timings', []))}\n\n"
        "Add each interval separately. Example:\n"
        "`Start 00:00:12 End 00:00:50`"
    )


def _blackbars_quality_kb(uid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("480p", callback_data=f"bb:{uid}:q:480"),
         InlineKeyboardButton("576p", callback_data=f"bb:{uid}:q:576")],
        [InlineKeyboardButton("720p", callback_data=f"bb:{uid}:q:720"),
         InlineKeyboardButton("1080p", callback_data=f"bb:{uid}:q:1080")],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"bb:{uid}:cancel")],
    ])


def _blackbars_mode_kb(uid: int, quality: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("16:9 BlackBars", callback_data=f"bb:{uid}:m:bars169")],
        [InlineKeyboardButton("4.5 BlackBars", callback_data=f"bb:{uid}:m:bars45")],
        [InlineKeyboardButton("16:9", callback_data=f"bb:{uid}:m:normal169")],
        [InlineKeyboardButton("◀️ Quality", callback_data=f"bb:{uid}:backq"),
         InlineKeyboardButton("❌ Cancel", callback_data=f"bb:{uid}:cancel")],
    ])


def _blackbars_timing_kb(uid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Add Timing", callback_data=f"bb:{uid}:add")],
        [InlineKeyboardButton("✅ Apply", callback_data=f"bb:{uid}:apply")],
        [InlineKeyboardButton("◀️ Mode", callback_data=f"bb:{uid}:backm"),
         InlineKeyboardButton("❌ Cancel", callback_data=f"bb:{uid}:cancel")],
    ])


def _blackbars_filter(mode: str, height: int, timings: list[tuple[int, int]]) -> str:
    """Build the complete video filter, including only the requested timings."""
    if mode == "bars169":
        # Exact 16:9 canvas at the selected output height. Fit the entire source;
        # the pad is what creates black bars and never crops/stretches the video.
        canvas_w = int(round(height * 16 / 9))
        vf = (
            f"scale=w={canvas_w}:h={height}:force_original_aspect_ratio=decrease:"
            f"force_divisible_by=2,pad={canvas_w}:{height}:(ow-iw)/2:(oh-ih)/2:black"
        )
    elif mode == "bars45":
        # Independent 4:5 implementation: fit the complete source inside a 4:5
        # canvas and pad with black where required.
        canvas_w = max(2, int(round(height * 4 / 5)))
        vf = (
            f"scale=w={canvas_w}:h={height}:force_original_aspect_ratio=decrease:"
            f"force_divisible_by=2,pad={canvas_w}:{height}:(ow-iw)/2:(oh-ih)/2:black"
        )
    else:
        # Normal 16:9 output: preserve the source aspect ratio and fit the
        # complete frame into the requested 16:9 canvas. No crop or stretch is
        # performed; padding is introduced only when the source itself is not
        # 16:9. A source that is already 16:9 therefore receives no bars.
        canvas_w = int(round(height * 16 / 9))
        vf = (
            f"scale=w={canvas_w}:h={height}:force_original_aspect_ratio=decrease:"
            f"force_divisible_by=2,pad={canvas_w}:{height}:(ow-iw)/2:(oh-ih)/2:black"
...
def _blackbars_watermark_enable(ranges: list[tuple[int, int]]) -> str:
    if not ranges:
        return "0"
    return "+".join(f"between(t,{start},{end})" for start, end in ranges)


def _blackbars_build_args(input_path: str, output_path: str, quality: str,
                          mode: str, timings: list[tuple[int, int]], wm_path: str) -> str:
    height = _BLACKBARS_QUALITY[quality]
    vf = _blackbars_filter(mode, height, timings)

    # The requested BlackBars watermark settings are deliberately local to this
    # command. Existing DishTV / Watermark positioning and configuration remain
    # untouched elsewhere in the bot.
    if timings:
        enable = _blackbars_watermark_enable(timings)
        fc = (
            f"[0:v]{vf}[base];"
            f"[1:v]scale=240:-1[wm];"
            f"[base][wm]overlay=20:H-h-60:enable='{enable}'[vout]"
        )
        return (
            f"-i {shlex.quote(input_path)} -loop 1 -i {shlex.quote(wm_path)} "
            f"-filter_complex {shlex.quote(fc)} -map '[vout]' -map 0:a? "
            f"-c:v libx264 -crf 23 -preset veryfast -c:a copy -shortest -movflags +faststart "
            f"{shlex.quote(output_path)}"
        )

    return (
        f"-i {shlex.quote(input_path)} -vf {shlex.quote(vf)} -map 0:v? -map 0:a? "
        f"-c:v libx264 -crf 23 -preset veryfast -c:a copy -movflags +faststart "
        f"{shlex.quote(output_path)}"
    )


def _blackbars_progress_text(job: dict) -> str:
    duration = float(job.get("duration") or 0)
    current = float(job.get("cur_time_sec") or 0)
    ...
    speed = float(job.get("speed_x") or 0)
    remaining = ((duration - current) / speed) if speed > 0 and duration > current else 0
    ...
    return (
        ...
        f"⏳ Remaining: `{_ff_sec_to_hms(remaining)}`\n"
        f"👤 Username: `{username}`\n"
        f"🆔 User ID: `{user_id}`"
    )


def _blackbars_progress_kb(task_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
...
async def _blackbars_run(client: Client, task_id: str) -> None:
    job = blackbars_jobs.get(task_id)
    if not job:
        return
    status_msg = job["status_msg"]
    proc = None
    ...
    stderr_tail: list[str] = []
    try:
        args = _blackbars_build_args(
            job["input_file"], job["output_file"], job["quality"],
            job["mode"], job["timings"], job["watermark_file"],
        )
        # -progress pipe:2 gives machine-readable out_time/speed updates while
        # stderr still carries useful FFmpeg diagnostics.
        proc = await asyncio.create_subprocess_shell(
            f"ffmpeg -y -hide_banner -nostats -progress pipe:2 {args}",
            stdout=asyncio.subprocess.DEVNULL,
            stderr=asyncio.subprocess.PIPE,
        )
        job["proc"] = proc
        ...
        while True:
            ...
            if "out_time_ms=" in line:
                try:
                    job["cur_time_sec"] = max(0.0, int(line.split("=", 1)[1]) / 1_000_000)
                except Exception:
                    pass
            elif "out_time=" in line:
                try:
                    job["cur_time_sec"] = _ff_hms_to_sec(line.split("=", 1)[1])
                except Exception:
                    pass
            elif line.startswith("speed="):
                raw_speed = line.split("=", 1)[1].strip().rstrip("x")
                try:
                    job["speed_x"] = float(raw_speed)
                except Exception:
                    pass
            ...
            if now - last_edit >= 5:
                try:
                    await status_msg.edit_text(
                        _blackbars_progress_text(job),
                        reply_markup=_blackbars_progress_kb(task_id),
                    )
                except Exception:
                    pass
                last_edit = now
        ...
        if job.get("cancelled"):
            ...
            try:
                ...
                pass
            if job.get("save_dir") and os.path.exists(job["save_dir"]):
                _safe_rmtree(job["save_dir"])
            return

        if rc != 0 or not os.path.exists(job["output_file"]) or os.path.getsize(job["output_file"]) <= 0:
            err = "\n".join(stderr_tail[-20:]) or "FFmpeg returned no diagnostic output."
            if len(err) > 2500:
                err = err[-2500:]
            job["status"] = "Failed"
            try:
        ...
        fname = os.path.basename(job["output_file"])
        try:
            await status_msg.edit_text(
        ...
        # Keep the SAME status message alive during upload. Telegram's media
        # message is separate; this message remains the single progress/status UI.
        upload_start = time.time()
        try:
            ...
            await split_and_send_video(
                ...
                status_msg=status_msg,
                progress=progress_for_pyrogram,
                progress_args=(status_msg, upload_start, status_msg, job["save_dir"], False),
                _uid=job["uid"],
                _chat_id=job["chat_id"],
            )
            await status_msg.edit_text(
                "✅ **Upload Completed**\n\n"
                f"📄 File:\n`{fname}`\n\n"
            ...
            schedule_retention_cleanup(job["save_dir"])
        ...
        job["status"] = "Failed"
        LOG.error("BlackBars task %s failed: %s", task_id, exc, exc_info=True)
        try:
            await status_msg.edit_text(f"❌ **Processing Failed**\n\n`{exc}`")
        except Exception:
            pass
        ...
        job["proc"] = None
        job["pid"] = None
        user_tasks.pop(job.get("uid"), None)


@app.on_message(filters.command(["BlackBars", "blackbars", "blackbar"]) & AUTH)
async def blackbars_cmd(client: Client, message: Message):
    uid = message.from_user.id
    if not is_verified(uid):
        return await message.reply_text("You must be **verified** to use /BlackBars. Run /verify.")
    if uid in user_tasks:
        return await message.reply_text("You already have an active job. Use /statusme or /cancelme.")

    src_msg, src_media = _get_replied_video(message)
    if not src_media:
        return await message.reply_text("**Reply to a video** with `/BlackBars` to start.")

    duration = int(getattr(src_media, "duration", 0) or 0)
    if duration <= 0:
        # Downloading is intentionally deferred until the user has selected all
        # processing options, but duration is needed to validate timing ranges.
        try:
            probe_dir = join(DOWNLOAD_DIRECTORY, f"blackbars_probe_{uid}_{int(time.time())}")
            os.makedirs(probe_dir, exist_ok=True)
            probe_path = await src_msg.download(file_name=join(probe_dir, f"src_{src_msg.id}"))
            duration = await get_duration_ffmpeg(probe_path)
            _safe_rmtree(probe_dir)
        except Exception:
            duration = 0

    if duration <= 0:
        return await message.reply_text("❌ Could not determine the video duration.")

    blackbars_sessions[uid] = {
        "uid": uid,
        "chat_id": message.chat.id,
        "src_msg_id": src_msg.id,
        "src_file_name": getattr(src_media, "file_name", None) or "video",
    ...
    await message.reply_text(
        "🎬 **BlackBars Setup**\n\n"
        f"📄 File: `{getattr(src_media, 'file_name', None) or 'video'}`\n"
        f"⏱ Duration: `{_ff_sec_to_hms(duration)}`\n\n"
        "Select output quality:",
        reply_markup=_blackbars_quality_kb(uid),
    )


@app.on_message(filters.text & AUTH)
async def blackbars_timing_text(client: Client, message: Message):
    uid = message.from_user.id if message.from_user else 0
    state = blackbars_sessions.get(uid)
    if not state or not state.get("awaiting_timing"):
        return

    parsed = _blackbars_parse_timing(message.text or "")
    if not parsed:
        return await message.reply_text(
            "❌ Invalid timing. Use:\n`Start 00:00:12 End 00:00:50`"
        )
    start, end = parsed
    candidate = list(state.get("timings", [])) + [(start, end)]
    error = _blackbars_validate_timing(candidate, state["duration"])
    if error:
        return await message.reply_text(f"❌ {error}")

    state["timings"] = sorted(candidate)
    state["awaiting_timing"] = False
    try:
        await state["status_msg"].edit_text(
            _blackbars_setup_text(state),
            reply_markup=_blackbars_timing_kb(uid),
        )
    except Exception:
        await message.reply_text(_blackbars_setup_text(state), reply_markup=_blackbars_timing_kb(uid))


@app.on_callback_query(filters.regex(r"^bb:(\d+):(q|m|add|apply|backq|backm|cancel)(?::([A-Za-z0-9_]+))?$"))
async def blackbars_setup_cb(client: Client, cq: CallbackQuery):
    parts = cq.data.split(":")
    uid = int(parts[1])
    action = parts[2]
    value = parts[3] if len(parts) > 3 else None
    if cq.from_user.id != uid:
        return await cq.answer("Not your BlackBars session.", show_alert=True, cache_time=0)

    state = blackbars_sessions.get(uid)
    if not state:
        return await cq.answer("Session expired.", show_alert=True, cache_time=0)

    if action == "cancel":
        blackbars_sessions.pop(uid, None)
        try:
            await cq.message.edit_text("❌ BlackBars setup cancelled.")
        except Exception:
            pass
        return await cq.answer("Cancelled.", cache_time=0)

    if action == "q":
        if value not in _BLACKBARS_QUALITY:
            return await cq.answer("Invalid quality.", show_alert=True, cache_time=0)
        state["quality"] = value
        return await cq.message.edit_text(
            f"🎬 **BlackBars Setup**\n\nSelected quality: `{value}p`\n\nSelect processing mode:",
            reply_markup=_blackbars_mode_kb(uid, value),
        )

    if action == "backq":
        return await cq.message.edit_text("🎬 **BlackBars Setup**\n\nSelect output quality:", reply_markup=_blackbars_quality_kb(uid))

    if action == "m":
        if value not in ("bars169", "bars45", "normal169"):
            return await cq.answer("Invalid mode.", show_alert=True, cache_time=0)
        state["mode"] = value
        state["timings"] = []
        state["awaiting_timing"] = False
        state["status_msg"] = cq.message
        return await cq.message.edit_text(
            _blackbars_setup_text(state),
            reply_markup=_blackbars_timing_kb(uid),
        )
    ...
    if action == "backm":
        return await cq.message.edit_text(
            f"🎬 **BlackBars Setup**\n\nSelected quality: `{state['quality']}p`\n\nSelect processing mode:",
            reply_markup=_blackbars_mode_kb(uid, state["quality"]),
        )

    if action == "add":
        state["awaiting_timing"] = True
        state["status_msg"] = cq.message
        await cq.answer("Send Start HH:MM:SS End HH:MM:SS", cache_time=0)
        return await cq.message.reply_text(
            "➕ **Add Watermark Timing**\n\n"
            "Send one range, for example:\n"
            "`Start 00:00:12 End 00:00:50`\n\n"
            "Start must be before End, and End must not exceed the video duration."
        )

    if action == "apply":
        if not state.get("quality") or not state.get("mode"):
            return await cq.answer("Select quality and mode first.", show_alert=True, cache_time=0)
        if not state.get("timings"):
            return await cq.answer("Add at least one watermark timing.", show_alert=True, cache_time=0)
        error = _blackbars_validate_timing(state["timings"], state["duration"])
        if error:
            return await cq.answer(error, show_alert=True, cache_time=0)

        await cq.answer("Starting BlackBars…", cache_time=0)
        status_msg = cq.message
        save_dir = join(DOWNLOAD_DIRECTORY, f"blackbars_{uid}_{int(time.time())}_{pysecrets.token_hex(3)}")
        os.makedirs(save_dir, exist_ok=True)
        try:
            await status_msg.edit_text("⬇️ Downloading video…")
            input_path = await client.download_media(
                message=await client.get_messages(status_msg.chat.id, state["src_msg_id"]),
                file_name=join(save_dir, f"src_{state['src_msg_id']}"),
            )
            if not input_path or not os.path.exists(input_path):
                raise RuntimeError("Video download failed.")

            duration = await get_duration_ffmpeg(input_path)
            if duration <= 0:
                duration = state["duration"]
            error = _blackbars_validate_timing(state["timings"], duration)
            if error:
                raise ValueError(error)

            if not await _async_ensure_watermark_img():
                raise RuntimeError("Configured watermark image could not be loaded.")
            wm_path = _watermark_img_path()
            if not wm_path:
                raise RuntimeError("Configured watermark image is unavailable.")

            filename = getattr(src_media, "file_name", None) or f"video_{state['src_msg_id']}.mp4"
            stem = os.path.splitext(os.path.basename(filename))[0] or "video"
            output_path = join(save_dir, f"{stem}_blackbars_{state['quality']}p.mp4")
            username = getattr(cq.from_user, "username", None) or getattr(cq.from_user, "first_name", None) or "Unknown"
            task_id = _blackbars_task_id(uid)
            blackbars_jobs[task_id] = {
                "uid": uid,
            ...
            await status_msg.edit_text(
                _blackbars_progress_text(blackbars_jobs[task_id]),
                reply_markup=_blackbars_progress_kb(task_id),
            )
            asyncio.create_task(_blackbars_run(client, task_id))
        except Exception as exc:
            _safe_rmtree(save_dir)
            try:
...
@app.on_callback_query(filters.regex(r"^progress:([A-Za-z0-9]+)$"))
async def blackbars_progress_cb(client: Client, cq: CallbackQuery):
    task_id = cq.data.split(":", 1)[1]
    job = blackbars_jobs.get(task_id)
    if not job:
        return await cq.answer("❌ Task not found or already expired.", show_alert=True, cache_time=0)
    if cq.from_user.id != job["uid"]:
        return await cq.answer("Not your task.", show_alert=True, cache_time=0)
    popup_text = _blackbars_progress_text(job)
    return await cq.answer(popup_text, show_alert=True, cache_time=0)


@app.on_callback_query(filters.regex(r"^cancel:([A-Za-z0-9]+)$"))
async def blackbars_cancel_cb(client: Client, cq: CallbackQuery):
    task_id = cq.data.split(":", 1)[1]
    job = blackbars_jobs.get(task_id)
    if not job:
    ...
    if cq.from_user.id != job["uid"]:
        return await cq.answer("Not your task.", show_alert=True, cache_time=0)
    if job.get("done"):
        return await cq.answer("Task already finished.", show_alert=True, cache_time=0)
    ...
    if proc and proc.returncode is None:
        try:
            ...
            pass
    await cq.answer("Cancellation requested.", cache_time=0)
    # The worker owns the final cancellation edit and removes only this task's
    # partial output. Do not touch any other user's FFmpeg process.
    try:
        await asyncio.wait_for(proc.wait(), timeout=2.0) if proc else None
    except Exception:
        pass
    try:
