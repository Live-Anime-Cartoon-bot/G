...
blackbars_tasks: dict = {}        # {task_id: /BlackBars selection + processing state}
...
async def runcmd(cmd: str) -> Tuple[int, str, str]:
    ...
    task_id = _processing_context.get()
    job = _processing_jobs.get(task_id) if task_id else None
    if job and args and os.path.basename(args[0]).lower().startswith("ffmpeg"):
        job["proc"] = process
        job["status"] = "Processing..."
        user_ffmpeg_pids[job["uid"]] = process.pid
...
async def do_record(client: Client, query: CallbackQuery, setup: dict):
    ...
    try:
        ...
        _processing_task_id = await _processing_start(
            msg, mkv_filename, user_id, duration=duration,
            username=query.from_user.username or "anonymous", phase="process"
        )
        _processing_bind(_processing_task_id)
        _processing_jobs[_processing_task_id]["save_dir"] = save_dir
        _processing_jobs[_processing_task_id]["output_file"] = video_path
        ...
        def _build_progress_text() -> str:
            ...
            if _processing_task_id in _processing_jobs:
                _processing_jobs[_processing_task_id]["percent"] = pct
                _processing_jobs[_processing_task_id]["current_sec"] = elapsed
                _processing_jobs[_processing_task_id]["status"] = "Recording..."
        ...
        async def update_recording_progress():
            while rec_id in active_recs.get(user_id, {}):
                if (user_id, rec_id) in cancelled_recs:
                    break
                kb = _processing_keyboard(_processing_task_id) if _processing_task_id in _processing_jobs else _rec_progress_kb(user_id, rec_id)
        ...
        await _processing_finish(_processing_task_id, video_path, upload=True)
...
async def handle_ott_download(client: Client, message: Message,
    ...
    try:
        ...
        _processing_task_id = await _processing_start(
            msg, os.path.basename(raw_filename or "download"), user_id,
            username=message.from_user.username or "anonymous", phase="process"
        )
        _processing_bind(_processing_task_id)
        _processing_jobs[_processing_task_id]["save_dir"] = save_dir
        ...
        def _hook(d: dict):
            ...
            if st == "downloading":
                ...
                _job = _processing_jobs.get(_processing_task_id)
                if _job:
                    _job["current"] = state["downloaded"]
                    _job["total"] = state["total"] or 0
                    _job["percent"] = state["percent"]
        ...
        async def watcher():
            while user_id in user_tasks:
                ...
                _job = _processing_jobs.get(_processing_task_id)
                if _job:
                    _job["percent"] = state.get("percent", 0.0)
                    _job["current"] = state.get("downloaded", 0)
                    _job["total"] = state.get("total") or 0
                    _job["status"] = "Downloading..." if state.get("status") != "finalizing" else "Finalizing..."
                    await _processing_edit(_job)
        ...
        if _job:
            _job["percent"] = 100.0
            _job["status"] = "Processing Completed"
...
async def run_compress(client: Client, status_msg: Message, state: dict):
    ...
    user_tasks[user_id]  = time.time()
    _processing_task_id = await _processing_start(status_msg, os.path.basename(src), user_id, duration=duration, phase="process")
    _processing_bind(_processing_task_id)
    _processing_jobs[_processing_task_id]["save_dir"] = save_dir
    _processing_jobs[_processing_task_id]["output_file"] = out_path
    user_status[user_id] = {"id": int(user_tasks[user_id]), "user_id": user_id,
    ...
    async def render():
        ...
        while proc.returncode is None:
            ...
            if _processing_task_id in _processing_jobs:
                _processing_jobs[_processing_task_id]["percent"] = pct
                _processing_jobs[_processing_task_id]["current_sec"] = done_sec
                _processing_jobs[_processing_task_id]["status"] = "Processing..."
    ...
    try:
        ...
        await _processing_finish(_processing_task_id, out_path, upload=True)
        _dest = await _await_upload_choice(
            user_id, status_msg,
            f"Compressed: `{out_size_mb:.1f} MB` (target `{target_mb} MB`)"
        )
        ...
        await _processing_fail(_processing_task_id, os.path.basename(src))
        ...
        await _processing_cleanup(_processing_task_id)
...
async def run_screenshots(client: Client, status_msg: Message, state: dict, n: int):
    ...
    try:
        ...
        for idx, ts in enumerate(timestamps, 1):
            ...
            if _processing_task_id in _processing_jobs:
                _processing_jobs[_processing_task_id]["percent"] = pct
                _processing_jobs[_processing_task_id]["current_sec"] = duration * pct / 100
                _processing_jobs[_processing_task_id]["status"] = "Processing..."
        ...
        await _processing_finish(_processing_task_id, upload=False)
...
async def run_merge(client: Client, message: Message, sess: dict):
    ...
    _processing_task_id = await _processing_start(status, os.path.basename(out_path), user_id, duration=total_dur, phase="process")
    _processing_bind(_processing_task_id)
    _processing_jobs[_processing_task_id]["save_dir"] = save_dir
    _processing_jobs[_processing_task_id]["output_file"] = out_path
    try:
        ...
        await _processing_fail(_processing_task_id, os.path.basename(out_path))
...
# ---------------------------------------------------------------------------
# Unified processing task progress (shared by video commands)
# ---------------------------------------------------------------------------
_processing_jobs: dict = {}
_processing_watchers: dict = {}
_processing_context = contextvars.ContextVar("processing_context", default=None)
_processing_edit_locks: dict = {}

def _new_processing_task_id(prefix="task"):
    return f"{prefix}_{uuid.uuid4().hex[:10]}"

def _processing_keyboard(task_id):
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("⚡ Progress", callback_data=f"progress:{task_id}"),
        InlineKeyboardButton("❌ Cancel", callback_data=f"blackbars_cancel:{task_id}"),
    ]])

def _processing_bar(pct, length=10):
    pct=max(0.0,min(100.0,float(pct)))
    n=int(round(pct/100*length))
    return "■"*n+"□"*(length-n)

def _processing_speed(job):
    elapsed=max(0.001,time.time()-job.get("speed_start",job.get("started_at",time.time())))
    if job.get("phase")=="upload":
        return (job.get("current",0) or 0)/elapsed
    if job.get("current_sec") is not None:
        return (job.get("current_sec",0) or 0)/elapsed
    return 0.0

def _processing_main_text(job):
    speed=_processing_speed(job)
    if job.get("phase")=="upload":
        speed_s=f"{speed/(1024*1024):.2f} MB/s" if speed else "Calculating..."
    else:
        speed_s=f"{speed:.2f}x" if speed else "Calculating..."
    return (f"🎬 Processing Video...\n\n📄 File:\n{job.get('filename','Video')}\n\n"
            f"Progress:\n[{_processing_bar(job.get('percent',0))}] {job.get('percent',0):.2f}%\n\n"
            f"⚡ Speed:\n{speed_s}\n\nStatus:\n{job.get('status','Processing...')}")

async def _processing_edit(job, text=None, markup=True):
    msg=job.get("message")
    if not msg: return
    lock=_processing_edit_locks.setdefault(job["task_id"],asyncio.Lock())
    async with lock:
        try:
            await msg.edit_text(text or _processing_main_text(job), reply_markup=_processing_keyboard(job["task_id"]) if markup else None)
        except Exception: pass

async def _processing_watcher(task_id):
    while task_id in _processing_jobs:
        job=_processing_jobs[task_id]
        if job.get("terminal"): return
        if job.get("phase") == "upload":
            await asyncio.sleep(5)
            continue
        proc=job.get("proc")
        ...
        if proc is not None and proc.returncode is None:
            if out and os.path.exists(out):
                job["current_bytes"]=os.path.getsize(out)
        await _processing_edit(job)
        await asyncio.sleep(5)

async def _processing_start(message, filename, uid, duration=0, username=None, phase="process", task_id=None):
    task_id=task_id or _new_processing_task_id("job")
    job={"task_id":task_id,"message":message,"uid":uid,"filename":filename or "Video",
         "duration":float(duration or 0),"started_at":time.time(),"speed_start":time.time(),
         "percent":0.0,"current_sec":0.0,"status":"Processing...","phase":phase,
         "current":0,"total":0,"proc":None,"output_file":None,"save_dir":None,
         "username":username or "anonymous","cancelled":False,"terminal":False}
    _processing_jobs[task_id]=job
    _processing_context.set(task_id)
    await _processing_edit(job)
    _processing_watchers[task_id]=asyncio.create_task(_processing_watcher(task_id))
    return task_id

def _processing_bind(task_id):
    return _processing_context.set(task_id)

async def _processing_finish(task_id, output_file=None, upload=False):
    job=_processing_jobs.get(task_id)
    if not job:return
    if output_file: job["output_file"]=output_file
    job["percent"]=100.0; job["status"]="Processing Completed"
    if upload:
        fname=os.path.basename(output_file) if output_file else job.get("filename","Video")
        await _processing_edit(job, f"✅ Processing Completed\n\n📄 File:\n{fname}\n\nUploading...", False)
        job["phase"]="upload"; job["status"]="Uploading..."; job["percent"]=0.0
        job["current"]=0; job["total"]=os.path.getsize(output_file) if output_file and os.path.exists(output_file) else 0
        job["speed_start"]=time.time()
        await _processing_edit(job)
    else:
        await _processing_edit(job)

async def _processing_fail(task_id, filename=None):
    job=_processing_jobs.get(task_id)
    if not job:return
    if job.get("cancelled"):
        return
    job["terminal"]=True; job["status"]="Failed"
    if filename:job["filename"]=filename
    await _processing_edit(job,f"❌ Processing Failed\n\n⚠️ Partial Output Deleted\n\n📄 File:\n{job.get('filename','Video')}",False)

async def _processing_cancel(task_id):
    job=_processing_jobs.get(task_id)
    if not job:return False
    job["cancelled"]=True; job["terminal"]=True
    proc=job.get("proc")
    if proc:
        try:
    ...
    if uid:
        ...
        pid=user_ffmpeg_pids.get(uid)
        if pid:
            try: psutil.Process(pid).terminate()
            except Exception: pass
    d=job.get("save_dir")
    if d and os.path.exists(d): _safe_rmtree(d)
    await _processing_edit(job,"❌ Process Cancelled\n\n⚠️ Partial Output Deleted",False)
    return True

async def _processing_cleanup(task_id):
    w=_processing_watchers.pop(task_id,None)
    if w and w is not asyncio.current_task(): w.cancel()
    _processing_edit_locks.pop(task_id,None)
    _processing_jobs.pop(task_id,None)

@app.on_callback_query(filters.regex(r"^progress:[A-Za-z0-9_]+$"))
async def unified_progress_callback(client, cq):
    task_id=cq.data.split(":",1)[1]; job=_processing_jobs.get(task_id)
    if not job:return await cq.answer("Task finished or expired.",show_alert=True,cache_time=0)
    if cq.from_user.id!=job.get("uid"):return await cq.answer("Not your task.",show_alert=True,cache_time=0)
    elapsed=int(time.time()-job.get("started_at",time.time()))
    pct=float(job.get("percent",0)); rem=int(job.get("duration",0)*(100-pct)/100) if job.get("duration") else 0
    speed=_processing_speed(job); speed_s=f"{speed/(1024*1024):.2f} MB/s" if job.get("phase")=="upload" else (f"{speed:.2f}x" if speed else "Calculating...")
    popup=(f"📄 File:\n{job.get('filename','Video')}\n\nProgress:\n[{_processing_bar(pct)}] {pct:.2f}%\n\n"
           f"Status:\n{job.get('status','Processing...')}\n\nElapsed:\n{_seconds_to_hms(elapsed)}\n\n"
           f"Remaining:\n{_seconds_to_hms(rem)}\n\nSpeed:\n{speed_s}\n\nUser:\n@{job.get('username','anonymous')}\n\nUser ID:\n{job.get('uid')}")
    await cq.answer(popup,show_alert=True,cache_time=0)

@app.on_callback_query(filters.regex(r"^cancel:[A-Za-z0-9_]+$"))
async def unified_cancel_callback(client, cq):
    task_id=cq.data.split(":",1)[1]; job=_processing_jobs.get(task_id)
    if not job:return await cq.answer("Task finished or expired.",show_alert=True,cache_time=0)
    if cq.from_user.id!=job.get("uid"):return await cq.answer("Not your task.",show_alert=True,cache_time=0)
    ok=await _processing_cancel(task_id)
    await cq.answer("Cancelled." if ok else "Task already finished.",cache_time=0)
...
async def progress_for_pyrogram(current, total, message, start, msg,
    ...
    now      = time.time()
    msg_key  = id(msg)
    unified_id = next((tid for tid,j in _processing_jobs.items() if j.get("message") is msg and j.get("uid") == getattr(getattr(msg, "from_user", None), "id", j.get("uid"))), None)
    if unified_id is None:
        unified_id = next((tid for tid,j in _processing_jobs.items() if j.get("message") is msg), None)
    unified_job = _processing_jobs.get(unified_id) if unified_id else None
    if unified_job:
        unified_job["phase"]="upload"
        unified_job["status"]="Uploading..."
        unified_job["current"]=current
        unified_job["total"]=total
        unified_job["percent"]=(current*100/total) if total else 0.0
        unified_job.setdefault("upload_started_at", now)
        unified_job["speed_start"]=unified_job.get("upload_started_at", now)
    ...
    if current == total:
        ...
        filename = os.path.basename(unified_job.get("output_file") or save_dir) if unified_job else (os.path.basename(save_dir) if save_dir else "File")
        size_str = f"{total/(1024**3):.1f} GB" if total >= 1024**3 else f"{total/(1024**2):.1f} MB"
        task_id  = _upload_task_id(msg_key)
        if unified_job:
            unified_job["percent"] = 100.0
            unified_job["status"] = "Upload Completed"
            unified_job["terminal"] = True
            final = (f"✅ Upload Completed\n\n📄 File:\n{filename}\n\n"
                     f"⏳ Server copy auto-deletes in {_retention_label()}.")
        else:
            final    = (
            f"✅ Sent Successfully\n\n"
    ...
    task_id = _upload_task_id(msg_key)
    if unified_job:
        _fire_edit(msg, _processing_main_text(unified_job))
    else:
    ...
    if action == "go":
        ...
        async def _ott_task():
            try:
                ...
                try:
                    if status_msg:
                        await status_msg.edit_text(f"❌ Processing Failed\n\n⚠️ Partial Output Deleted\n\n📄 File:\n{_filename or 'download'}")
                    else:
                        await orig_message.edit_text(f"❌ Processing Failed\n\n⚠️ Partial Output Deleted\n\n📄 File:\n{_filename or 'download'}")
                except Exception:
                    pass
...
async def trim_cmd(client: Client, message: Message):
    ...
    try:
        ...
        user_tasks[user_id]  = time.time()
        _processing_task_id = await _processing_start(status, os.path.basename(dl_path), user_id, duration=clip_len, phase="process")
        _processing_bind(_processing_task_id)
        _processing_jobs[_processing_task_id]["save_dir"] = save_dir
        _processing_jobs[_processing_task_id]["output_file"] = out_path
        user_status[user_id] = {
        ...
        if "_processing_task_id" in locals(): await _processing_fail(_processing_task_id, os.path.basename(dl_path) if "dl_path" in locals() else "Video")
        ...
        if "_processing_task_id" in locals(): await _processing_cleanup(_processing_task_id)
        user_status.pop(user_id, None)
        user_tasks.pop(user_id, None)
        progress_tasks.pop(user_id, None)
        cancelled_users.discard(user_id)
...
# ---------------------------------------------------------------------------
# /BlackBars — quality + mode selector (callback-safe task workflow)
# ---------------------------------------------------------------------------
_BLACKBARS_QUALITY = {
    "480":  (854, 480),
    "576":  (1024, 576),
...
def _blackbars_menu_keyboard(task_id: str, state: dict) -> InlineKeyboardMarkup:
    """BlackBars menu. Every callback is task-scoped and unique."""
    q = state.get("quality")
    mode = state.get("mode")

    def qbtn(value: str, label: str):
        mark = "✅ " if q == value else ""
        return InlineKeyboardButton(
            f"{mark}{label}",
            callback_data=f"blackbars_quality:{task_id}:{value}",
        )

    def mbtn(value: str, label: str):
        mark = "✅ " if mode == value else ""
        return InlineKeyboardButton(
            f"{mark}{label}",
            callback_data=f"blackbars_mode:{task_id}:{value}",
        )

    return InlineKeyboardMarkup([
        [qbtn("480", "480p"), qbtn("576", "576p")],
        [qbtn("720", "720p"), qbtn("1080", "1080p")],
        [mbtn("169", "16:9 BlackBars")],
        [mbtn("45", "4.5 BlackBars")],
        [mbtn("normal169", "16:9")],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"cancel:{task_id}")],
    ])


def _blackbars_menu_text(state: dict) -> str:
    duration = float(state.get("duration") or 0)
    duration_text = _seconds_to_hms(int(duration)) if duration > 0 else "00:00:00"
    q = state.get("quality") or "Not selected"
    mode = _BLACKBARS_MODES.get(state.get("mode"), "Not selected")
    return (
        "⬛ **/BlackBars**\n\n"
        f"Video `{duration_text}`\n\n"
        f"Quality: `{q}p`\n"
        f"Mode: `{mode}`\n\n"
        "Select Quality and BlackBars Mode.\n"
        "FFmpeg starts automatically after both are selected."
    )


def _blackbars_filter_for(state: dict) -> str:
    """Build only the existing BlackBars video framing filter."""
    q = state["quality"]
    mode = state["mode"]
    out_w, out_h = _BLACKBARS_QUALITY[q]

    if mode == "169":
        # Preserve the complete frame; pad with black instead of cropping/stretching.
        return (
    ...
    if mode == "45":
        # Existing project's 4:5/4.5 BlackBars implementation.
        canvas_w = max(2, round(out_h * 4 / 5))
        return (
            f"scale={canvas_w}:{out_h}:force_original_aspect_ratio=decrease,"
            f"pad={canvas_w}:{out_h}:(ow-iw)/2:(oh-ih)/2:black,setsar=1"
        )

    # Existing normal 16:9 implementation: fill the selected 16:9 frame.
    return (
...
async def _blackbars_run_ffmpeg(task_id: str, input_file: str, output_file: str) -> None:
    """Run BlackBars FFmpeg with live -progress output tied to task_id."""
    job = _processing_jobs.get(task_id)
    if not job:
        return

    state = job.get("blackbars_state") or {}
    vf = _blackbars_filter_for(state)
    ...
    LOG.info(
        "BlackBars callback processing started: task=%s uid=%s quality=%sp mode=%s",
        task_id, job.get("uid"), state.get("quality"), state.get("mode"),
    )
    ...
    try:
        while True:
            ...
            if key in ("out_time_us", "out_time_ms"):
                try:
                    # FFmpeg's progress field is microseconds on normal builds.
                    current_sec = float(value) / 1_000_000.0
                    job["current_sec"] = max(0.0, current_sec)
                    if duration > 0:
                        job["percent"] = min(100.0, current_sec * 100.0 / duration)
                except (TypeError, ValueError):
                    pass
            elif key == "progress" and value == "end":
                job["current_sec"] = duration
                job["percent"] = 100.0
        ...
        if rc != 0:
            raise RuntimeError(
                f"FFmpeg exited with code {rc}:\n{stderr.strip()[-1800:] or 'no stderr'}"
            )
        if not os.path.exists(output_file) or os.path.getsize(output_file) <= 0:
            raise RuntimeError("FFmpeg completed but produced an empty output file.")
        ...
        if job.get("proc") is proc:
            job["proc"] = None
        if user_ffmpeg_pids.get(job.get("uid")) == proc.pid:
            user_ffmpeg_pids.pop(job.get("uid"), None)


async def _blackbars_process_task(client: Client, task_id: str) -> None:
    """Background BlackBars processing + existing upload workflow."""
    job = _processing_jobs.get(task_id)
    if not job:
        return
    ...
    # Make the task id available to runcmd/progress helpers if any existing
    # helper is called by this workflow.
    _processing_bind(task_id)

    try:
        await _blackbars_run_ffmpeg(task_id, input_file, output_file)
        job = _processing_jobs.get(task_id)
        if not job:
            return
        if job.get("cancelled"):
        ...
        rc, _out, err = await runcmd(
            f'ffmpeg -hide_banner -loglevel error -nostats -y '
            f'-ss {max(0, int(job.get("duration", 0) // 2))} '
            f'-i {shlex.quote(output_file)} '
            f'-vframes 1 -q:v 2 {shlex.quote(thumb)}'
        )
        thumb_path = thumb if rc == 0 and os.path.exists(thumb) else None

        caption = (
            "⬛ **BlackBars Applied**\n\n"
            f"Quality: `{job['blackbars_state']['quality']}p`\n"
            f"Mode: `{_BLACKBARS_MODES[job['blackbars_state']['mode']]}`\n"
            f"Size: `{out_mb:.1f} MB`\n"
            f"Duration: `{_seconds_to_hms(int(job.get('duration', 0)))}`\n"
            f"_Auto-deleted from server after {_retention_label()}._"
        )

        await _processing_finish(task_id, output_file, upload=True)
        await _run_upload_destination(
            client, uid, "tg", job["message"], output_file, caption,
            int(job.get("duration", 0)), thumb_path=thumb_path, save_dir=save_dir,
        )
        success = True
        ...
        if job and job.get("cancelled"):
            return
        LOG.error("BlackBars processing failed task=%s uid=%s: %s", task_id, uid, exc)
        await _processing_fail(task_id, job.get("filename") if job else "video")

    finally:
        cancelled_users.discard(uid)
        user_tasks.pop(uid, None)
        blackbars_tasks.pop(task_id, None)
        if task_id in _processing_jobs:
            await _processing_cleanup(task_id)
        if save_dir and os.path.exists(save_dir):
            if success:
                schedule_retention_cleanup(save_dir)
            else:
                # _processing_cancel already deletes partial output/save_dir.
                if not job or not job.get("cancelled"):
                    _safe_rmtree(save_dir)
...
async def blackbars_cmd(client: Client, message: Message):
    ...
    task_id = _new_processing_task_id("bb")
    save_dir = join(DOWNLOAD_DIRECTORY, f"blackbars_{task_id}")
    os.makedirs(save_dir, exist_ok=True)
    status = await message.reply_text("⬛ Preparing /BlackBars…")
    ...
    blackbars_tasks[task_id] = state

    try:
        # Download/probe once so the menu can show the real video duration.
        dl_path = await src_msg.download(file_name=join(save_dir, f"src_{src_msg.id}"))
        state["input_file"] = dl_path
        info = await _ffprobe_video(dl_path)
        duration = float(info.get("duration") or 0)
        if duration <= 0:
            raise RuntimeError("Could not determine source video duration.")
        state["duration"] = duration
        state["filename"] = os.path.basename(dl_path)

        await status.edit_text(
            _blackbars_menu_text(state),
            reply_markup=_blackbars_menu_keyboard(task_id, state),
        )
    except Exception as exc:
        LOG.error("BlackBars setup failed task=%s uid=%s: %s", task_id, user_id, exc)
        blackbars_tasks.pop(task_id, None)
        _safe_rmtree(save_dir)
        try:
            await status.edit_text(f"❌ **/BlackBars failed.**\n\n`{exc}`")
        except Exception:
            pass


@app.on_callback_query(filters.regex(r"^blackbars_cancel:[A-Za-z0-9_]+$"))
async def blackbars_menu_cancel_callback(client: Client, cq: CallbackQuery):
    task_id = cq.data.split(":", 1)[1]
    LOG.info("BlackBars callback received: %s", cq.data)
    state = blackbars_tasks.get(task_id)
    if not state:
        return await cq.answer("BlackBars task expired.", show_alert=True, cache_time=0)
    if cq.from_user.id != state["user_id"]:
        return await cq.answer("Not your BlackBars task.", show_alert=True, cache_time=0)

    await cq.answer("Cancelled.", cache_time=0)
    blackbars_tasks.pop(task_id, None)
    user_tasks.pop(state["user_id"], None)
    _safe_rmtree(state.get("save_dir"))
    try:
        await cq.message.edit_text("❌ /BlackBars cancelled.", reply_markup=None)
    except Exception:
        pass


@app.on_callback_query(filters.regex(r"^blackbars_quality:[A-Za-z0-9_]+:(480|576|720|1080)$"))
async def blackbars_quality_callback(client: Client, cq: CallbackQuery):
    data = cq.data.split(":")
    task_id = data[1]
    quality = data[2]
    LOG.info("BlackBars callback received: %s", cq.data)
    ...
    if state.get("started"):
        return await cq.answer("Processing already started.", cache_time=0)
    ...
    if state.get("mode") and not state.get("started"):
        state["started"] = True
        user_tasks[state["user_id"]] = time.time()
        job_task_id = await _processing_start(
            cq.message,
            state.get("filename") or "video",
            state["user_id"],
            duration=state["duration"],
            username=cq.from_user.username or "anonymous",
            phase="process",
            task_id=task_id,
        )
        job = _processing_jobs[job_task_id]
        job["save_dir"] = state["save_dir"]
        job["input_file"] = state["input_file"]
        job["output_file"] = join(
            state["save_dir"],
            f"BlackBars_{quality}p_{task_id}.mp4",
        )
        job["blackbars_state"] = state.copy()
        state["output_file"] = job["output_file"]
        blackbars_tasks.pop(task_id, None)
        asyncio.create_task(_blackbars_process_task(client, task_id))


@app.on_callback_query(filters.regex(r"^blackbars_mode:[A-Za-z0-9_]+:(169|45|normal169)$"))
async def blackbars_mode_callback(client: Client, cq: CallbackQuery):
    data = cq.data.split(":")
    task_id = data[1]
    mode = data[2]
    LOG.info("BlackBars callback received: %s", cq.data)
    ...
    if state.get("quality") and not state.get("started"):
        state["started"] = True
        user_tasks[state["user_id"]] = time.time()
        job_task_id = await _processing_start(
            cq.message,
        ...
        job["output_file"] = join(
            state["save_dir"],
            f"BlackBars_{state['quality']}p_{task_id}.mp4",
        )
        job["blackbars_state"] = state.copy()
        state["output_file"] = job["output_file"]
        blackbars_tasks.pop(task_id, None)
...
async def wmp_callback(client: Client, cq: CallbackQuery):
    ...
    _processing_task_id = await _processing_start(status, "watermark_video", uid, phase="process")
    _processing_bind(_processing_task_id)
    _processing_jobs[_processing_task_id]["save_dir"] = save_dir
    os.makedirs(save_dir, exist_ok=True)
    user_tasks[uid] = True
    ...
    try:
        ...
        if _processing_task_id in _processing_jobs: await _processing_finish(_processing_task_id, out_path, upload=True)
        ...
        if "_processing_task_id" in locals(): await _processing_fail(_processing_task_id, "watermark_video")
...
async def audiotrack_cmd(client: Client, message: Message):
    ...
    _processing_task_id = await _processing_start(status, "video", user_id, phase="process")
    _processing_bind(_processing_task_id)
    _processing_jobs[_processing_task_id]["save_dir"] = save_dir
    ...
    try:
        ...
        if "_processing_task_id" in locals(): await _processing_fail(_processing_task_id, "video")
